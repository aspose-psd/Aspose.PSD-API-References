...
description: SmartObjectLayer méthode. Remplace le contenu de lobjet intelligent intégré dans la couche dobjet intelligent.
type: docs
weight: 140
url: /fr/net/aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/
---
## ReplaceContents(Image) {#replacecontents}

Remplace le contenu de l'objet intelligent intégré dans la couche d'objet intelligent.
...
| --- | --- | --- |
| image | Image | L'image. |
| resolution | ResolutionSetting | Les paramètres de résolution. Si null, la résolution de l'image sera utilisée. |

### Exceptions

| exception | condition |
| --- | --- |
| [PsdImageException](../../../aspose.psd.coreexceptions.imageformats/psdimageexception/) | Ne peut remplacer que l'objet intelligent intégré. |

### Exemples

Cet exemple montre que la méthode ReplaceContents fonctionne correctement lorsque le nouveau fichier de contenu a une résolution différente.
...
string filePath = baseFolder + fileName; // image PSD d'origine
string newContentPath = baseFolder + "image.jpg"; // le nouveau fichier de contenu pour l'objet intelligent
string outputFilePath = outputFolder + "ChangedPsd";
string pngOutputPath = outputFilePath + ".png"; // le fichier PNG de sortie
string psdOutputPath = outputFilePath + ".psd"; // le fichier PSD de sortie
