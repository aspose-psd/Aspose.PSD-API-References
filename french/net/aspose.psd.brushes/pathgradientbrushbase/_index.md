...
description: Aspose.PSD.Brushes.PathGradientBrushBase classe. Représente unBrush avec fonctionnalité de gradient de chemin de base.
type: docs
weight: 180
...
Notez que lors de la création du`PathGradientBrushBase` class il doit être initialisé avec 2 points au moins. Le chemin interne created sera toujours une figure fermée, le dernier point relie le premier point. Cette forme est remplie de ceci`PathGradientBrushBase`. L'implémentation de GDI+ lance uneOutOfMemoryException lors du passage dans des tableaux vides ou ensemble de points ayant les mêmes coordonnées. Le`PathGradientBrushBase` lève une exception lorsque le tableau de points contient moins de 2 points, leArgumentException is lancé plutôt queOutOfMemoryException lorsque le tableau de points est inacceptable. Le point central est calculé comme centre de masse pour les points transmis par défaut. Un utilisateur peut modifier ce point ultérieurement. L'échelle de mise au point est un point vide (0.0, 0.0) par défaut.
