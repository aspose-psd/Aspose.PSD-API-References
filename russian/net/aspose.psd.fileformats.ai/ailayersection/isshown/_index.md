...
description: Получает или задает значение указывающее отображается ли этот слой. Отображает все изображения содержащиеся в слое на монтажной области если установлено значение true.
type: docs
weight: 90
...
Получает или задает значение, указывающее, отображается ли этот слой. Отображает все изображения, содержащиеся в слое, на монтажной области, если установлено значение true.
...
` true` если этот слой отображается; в противном случае` false` .

### Примеры

Следующий пример демонстрирует поддержку слоев в файлах формата AI.
