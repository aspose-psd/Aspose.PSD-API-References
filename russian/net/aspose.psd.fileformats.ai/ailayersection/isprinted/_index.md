...
description: Получает или задает значение указывающее печатается ли этот слой. Делает иллюстрацию содержащуюся в слое печатаемой если true.
type: docs
weight: 80
...
` true` если этот слой печатается; в противном случае` false` .
