---
title: PdfPackage
second_title: Справочник по Aspose.PSD для .NET API
description: Представляет пространство имен Adobe Pdf.
type: docs
weight: 5670
url: /ru/net/aspose.psd.xmp.schemas.pdf/pdfpackage/
---
## PdfPackage class

Представляет пространство имен Adobe Pdf.

```csharp
public sealed class PdfPackage : XmpPackage
```

## Конструкторы

| Имя | Описание |
| --- | --- |
| [PdfPackage](pdfpackage)() | Конструктор по умолчанию. |

## Характеристики

| Имя | Описание |
| --- | --- |
| virtual [Item](../../aspose.psd.xmp/xmppackage/item) { get; set; } | Получает или задаетObjectс указанным ключом. |
| virtual [Keys](../../aspose.psd.xmp/xmppackage/keys) { get; } | Получает ключи в пакете XMP. |
| [NamespaceUri](../../aspose.psd.xmp/xmppackage/namespaceuri) { get; } | Получает URI пространства имен. |
| [Prefix](../../aspose.psd.xmp/xmppackage/prefix) { get; } | Получает префикс. |
| [XmlNamespace](../../aspose.psd.xmp/xmppackage/xmlnamespace) { get; } | Получает пространство имен XML. |

## Методы

| Имя | Описание |
| --- | --- |
| override [AddValue](../../aspose.psd.xmp.schemas.pdf/pdfpackage/addvalue)(string, string) | Добавляет строковое свойство. |
| virtual [Clear](../../aspose.psd.xmp/xmppackage/clear)() | Очищает этот экземпляр. |
| virtual [ContainsKey](../../aspose.psd.xmp/xmppackage/containskey)(string) | Определяет, содержит ли указанный ключ ключ. |
| [GetEnumerator](../../aspose.psd.xmp/xmppackage/getenumerator)() | Возвращает перечислитель, который перебирает коллекцию. |
| virtual [GetXmlValue](../../aspose.psd.xmp/xmppackage/getxmlvalue)() | Преобразует значение XMP в представление XML. |
| virtual [Remove](../../aspose.psd.xmp/xmppackage/remove)(string) | Удалить значение с указанным ключом. |
| [SetKeywords](../../aspose.psd.xmp.schemas.pdf/pdfpackage/setkeywords)(string) | Устанавливает ключевые слова. |
| [SetPdfVersion](../../aspose.psd.xmp.schemas.pdf/pdfpackage/setpdfversion)(string) | Устанавливает версию PDF. |
| [SetProducer](../../aspose.psd.xmp.schemas.pdf/pdfpackage/setproducer)(string) | Устанавливает имя инструмента, создавшего Pdf. |
| [SetTrapped](../../aspose.psd.xmp.schemas.pdf/pdfpackage/settrapped)(bool) | Устанавливает в ловушку. |
| virtual [SetValue](../../aspose.psd.xmp/xmppackage/setvalue)(string, IXmlValue) | Устанавливает значение. |
| virtual [SetXmpTypeValue](../../aspose.psd.xmp/xmppackage/setxmptypevalue)(string, XmpTypeBase) | Устанавливает значение типа XMP. |

### Смотрите также

* class [XmpPackage](../../aspose.psd.xmp/xmppackage)
* пространство имен [Aspose.PSD.Xmp.Schemas.Pdf](../../aspose.psd.xmp.schemas.pdf)
* сборка [Aspose.PSD](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
