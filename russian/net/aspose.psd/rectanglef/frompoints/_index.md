---
title: FromPoints
second_title: Справочник по Aspose.PSD для .NET API
description: Создает новыйRectangleaspose.psd/rectangle из двух указанных точек. Две вершины созданногоRectangleaspose.psd/rectangle будет равно переданномуpoint1 а такжеpoint2 . Обычно это противоположные вершины.
type: docs
weight: 40
url: /ru/net/aspose.psd/rectanglef/frompoints/
---
## RectangleF.FromPoints method

Создает новый[`Rectangle`](../../rectangle) из двух указанных точек. Две вершины созданного[`Rectangle`](../../rectangle) будет равно переданному*point1* а также*point2* . Обычно это противоположные вершины.

```csharp
public static RectangleF FromPoints(PointF point1, PointF point2)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| point1 | PointF | Первый[`Point`](../../point) для нового прямоугольника. |
| point2 | PointF | Секунда[`Point`](../../point) для нового прямоугольника. |

### Возвращаемое значение

Недавно созданный[`Rectangle`](../../rectangle).

### Смотрите также

* struct [PointF](../../pointf)
* struct [RectangleF](../../rectanglef)
* пространство имен [Aspose.PSD](../../rectanglef)
* сборка [Aspose.PSD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
