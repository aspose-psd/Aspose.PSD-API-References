...
type: docs
weight: 15
url: /ru/java/com.aspose.psd.brushes/pathgradientbrushbase/
---
...
Представляет Brush с функцией градиента базового пути.

Обратите внимание, что при создании класса  PathGradientBrushBase  его следует инициализировать как минимум двумя точками. Внутренний путь всегда будет замкнутой фигурой, последняя точка соединяется с первой. Эта фигура заполняется этим  PathGradientBrushBase . Реализация GDI+ бросает  OutOfMemoryError  при передаче пустых массивов или набора точек с одинаковыми координатами.  PathGradientBrushBase  бросает исключение, когда массив точек содержит менее 2 точек; в этом случае бросается  ArgumentException , а не  OutOfMemoryError , если массив точек неприемлем. Точка центра вычисляется как центр масс переданных точек по умолчанию. Пользователь может изменить эту точку позже. Масштаб фокуса по умолчанию — пустая точка (0.0, 0.0).
## Методы
