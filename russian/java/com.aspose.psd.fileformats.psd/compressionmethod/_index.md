...
type: docs
weight: 11
url: /ru/java/com.aspose.psd.fileformats.psd/compressionmethod/
---
...
| [RLE](#RLE) | При RLE‑сжатии данные изображения начинаются с подсчётов байтов для всех строк сканирования (строки \* каналы), при этом каждый подсчёт хранится как двухбайтовое значение. |
| [Raw](#Raw) | Без сжатия. |
...
При RLE‑сжатии данные изображения начинаются с подсчётов байтов для всех строк сканирования (строки \* каналы), при этом каждый подсчёт хранится как двухбайтовое значение. Затем следуют RLE‑сжатые данные, при этом каждая строка сканирования сжимается отдельно. RLE‑сжатие использует тот же алгоритм сжатия, что и процедура PackBits в ROM Macintosh и стандарт TIFF.
...
Без сжатия. Данные изображения хранятся как необработанные байты в планарном порядке RGBA. Это означает, что сначала записываются все данные R, затем все G, затем все B и, наконец, все данные A.
