...
description: Aspose.PSD.FileFormats.Psd.SmartObjectProvider klas. Definieert de slimme objectprovider die zorgt voor het ophalen/instellen van gegevensbronnen van globale linkbronnen van het PSDbestand en hun inhoud.
type: docs
weight: 4000
url: /nl/net/aspose.psd.fileformats.psd/smartobjectprovider/
---
## SmartObjectProvider class

Definieert de slimme objectprovider die zorgt voor het ophalen/instellen van gegevensbronnen van globale linkbronnen van het PSD-bestand en hun inhoud.
...
| Naam | Beschrijving |
| --- | --- |
| [ConvertToSmartObject](../../aspose.psd.fileformats.psd/smartobjectprovider/converttosmartobject/#converttosmartobject_1)(params int[]) | Converteert lagen naar een ingebed slim object. |
| [ConvertToSmartObject](../../aspose.psd.fileformats.psd/smartobjectprovider/converttosmartobject/#converttosmartobject)(Layer[]) | Converteert lagen naar een ingebed slim object. |
| [EmbedAllLinked](../../aspose.psd.fileformats.psd/smartobjectprovider/embedalllinked/)() | Sluit alle gekoppelde slimme objecten in de afbeelding in. |
| [NewSmartObjectViaCopy](../../aspose.psd.fileformats.psd/smartobjectprovider/newsmartobjectviacopy/)(SmartObjectLayer) | Maakt een nieuwe slimme objectlaag door de bronlaag te kopiëren. |
| [UpdateAllModifiedContent](../../aspose.psd.fileformats.psd/smartobjectprovider/updateallmodifiedcontent/)() | Werkt de inhoud bij van alle gewijzigde slimme objecten in de afbeelding. |

### Voorbeelden

De volgende code demonstreert de ondersteuning van het bijwerken van gekoppelde slimme objecten.
...
// Dit voorbeeld laat zien hoe u de externe of ingesloten slimme objectlaag kunt bijwerken met behulp van deze methoden:
...
void ExampleOfUpdatingSmartObjectLayer(
    ...
    FileFormat format)
{
    // Dit voorbeeld laat zien hoe u de slimme objectlaag in het PSD-bestand kunt wijzigen en de inhoud ervan kunt exporteren/bijwerken.
    ...
    using (PsdImage image = (PsdImage)Image.Load(filePath))
    {
        ...
        if (contentType == SmartObjectType.AvailableLinked)
        {
            ...
            // Laten we de externe slimme objectafbeelding van de PSD slimme objectlaag naar een nieuwe locatie exporteren
            // omdat we het gaan wijzigen.
            smartObjectLayer.ExportContents(exportPath);
            smartObjectLayer.RelinkToFile(exportPath);
        }

        // Laten we de inhoud van het slimme object omkeren: binnenste (niet in de cache opgeslagen) afbeelding
        ...
        // Laten we eens kijken of de gewijzigde inhoud de weergave nog niet beïnvloedt.
        image.Save(pngOutputPath, new PngOptions() { ColorType = PngColorType.TruecolorWithAlpha });

        smartObjectLayer.UpdateModifiedContent();

        // Laten we eens kijken of de bijgewerkte inhoud van invloed is op de weergave en of de psd-afbeelding correct is opgeslagen
        image.Save(psd2OutputPath, new PsdOptions(image));
        image.Save(png2OutputPath, new PngOptions() { ColorType = PngColorType.TruecolorWithAlpha });
    }
}

// Dit voorbeeld laat zien hoe het ingebedde slimme object kan worden geconverteerd naar externe gekoppelde inhoud met behulp van de methode ConvertToLinked.
...
void ExampleOfEmbeddedSmartObjectLayerToLinkedConversion(
    ...
    FileFormat format)
{
    // Dit laat zien hoe u een ingebedde slimme objectlaag in het PSD-bestand converteert naar een externe laag.
    ...
    using (PsdImage image = (PsdImage)Image.Load(filePath))
    {
        ...
        // Laten we eens kijken of de geconverteerde afbeelding correct is opgeslagen
...
// Dit voorbeeld laat zien hoe u één externe slimme objectlaag of alle gekoppelde lagen in het PSD-bestand kunt insluiten met behulp van de EmbedLinked-methode.
...
// Dit voorbeeld laat zien hoe u de externe slimme objectlaag van Adobe® Photoshop® wijzigt en de inhoud ervan exporteert/bijwerkt
// met de methoden ExportContents en ReplaceContents.
...
void ExampleOfExternalSmartObjectLayerSupport(string filePath, int contentsLength, int left, int top, int right, int bottom, FileFormat format)
{
    ...
    using (PsdImage image = (PsdImage)Image.Load(filePath))
    {
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(exportPath));
        // Laten we de gekoppelde slimme objectafbeelding exporteren vanuit de PSD slimme objectlaag
        smartObjectLayer.ExportContents(exportPath);
        ...
        using (var innerImage = (RasterImage)smartObjectLayer.LoadContents(null))
        {
            ...
            // Laten we de gekoppelde afbeelding van het slimme object omkeren
            InvertImage(innerImage);
            innerImage.Save(linkOutputPath);

            // Laten we de gekoppelde afbeelding van het slimme object in de PSD-laag vervangen
