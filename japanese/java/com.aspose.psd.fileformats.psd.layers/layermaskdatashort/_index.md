...
description: "LayerMaskDataShort クラスを定義します。このクラスは、レイヤーがラスターマスクまたはベクトルマスクのいずれか一方のみを持つ場合の PSD ファイルレイヤー内のマスクデータに関する情報を含みます。"
type: docs
weight: 23
url: /ja/java/com.aspose.psd.fileformats.psd.layers/layermaskdatashort/
---
...
LayerMaskDataShort クラスを定義します。このクラスは、レイヤーがラスターマスクまたはベクターマスクのいずれかのみを持ち、両方を持たない場合の PSD ファイルレイヤー内のマスクデータに関する情報を含みます。そうでない場合は、[LayerMaskDataFull](../../com.aspose.psd.fileformats.psd.layers/layermaskdatafull) が使用されます。レイヤーがラスターマスクのみを持つ場合、ImageData にはラスターマスクデータのバイトが含まれます。レイヤーがベクターマスクのみを持つ場合、ImageData にはベクターマスクがラスタライズ（キャッシュ）されたデータバイトが含まれます。LayerMaskData.ImageData（[LayerMaskData.getImageData](../../com.aspose.psd.fileformats.psd.layers/layermaskdata\\#getImageData)/[LayerMaskData.setImageData(byte[])](../../com.aspose.psd.fileformats.psd.layers/layermaskdata\\#setImageData-byte---)）のバイト長は、Width \\* Height が LayerMaskData.MaskRectangle（[LayerMaskData.getMaskRectangle](../../com.aspose.psd.fileformats.psd.layers/layermaskdata\\#getMaskRectangle)/[LayerMaskData.setMaskRectangle(Rectangle)](../../com.aspose.psd.fileformats.psd.layers/layermaskdata\\#setMaskRectangle-Rectangle-)）プロパティと等しくなる必要があります。
## Constructors

| Constructor | 説明 |
| --- | --- |
| [LayerMaskDataShort()](#LayerMaskDataShort--) | 新しい [LayerMaskDataShort](../../com.aspose.psd.fileformats.psd.layers/layermaskdatashort) クラスのインスタンスを初期化します。 |
## メソッド
...
| [getPadding()](#getPadding--) | レイヤーマスクのパディングを取得または設定します。 |
...
| [setPadding(short value)](#setPadding-short-) | レイヤーマスクのパディングを取得または設定します。 |
...
新しい [LayerMaskDataShort](../../com.aspose.psd.fileformats.psd.layers/layermaskdatashort) クラスのインスタンスを初期化します。
