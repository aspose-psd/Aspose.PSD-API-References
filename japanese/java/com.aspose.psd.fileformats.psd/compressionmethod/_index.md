...
type: docs
weight: 11
url: /ja/java/com.aspose.psd.fileformats.psd/compressionmethod/
---
...
| [RLE](#RLE) | RLE 圧縮された画像データは、すべての走査線（行 \* チャネル）のバイト数で始まり、各カウントは 2 バイトの値として格納されます。 |
| [Raw](#Raw) | 圧縮なし。 |
| [ZipWithPrediction](#ZipWithPrediction) | 予測付き ZIP。 |
| [ZipWithoutPrediction](#ZipWithoutPrediction) | 予測なし ZIP。 |
## メソッド
...
RLE 圧縮された画像データは、すべての走査線（行 \* チャネル）のバイト数で始まり、各カウントは 2 バイトの値として格納されます。その後に RLE 圧縮データが続き、各走査線は個別に圧縮されます。RLE 圧縮は、Macintosh ROM の PackBits ルーチンおよび TIFF 標準で使用されているのと同じ圧縮アルゴリズムです。
...
圧縮なし。画像データは RGBA 平面順で生バイトとして格納されます。つまり、まずすべての R データが書き込まれ、次に G、続いて B、最後に A データが書き込まれます。
