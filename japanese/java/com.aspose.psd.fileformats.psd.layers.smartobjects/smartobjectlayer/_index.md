...
description: "SmartObjectLayer クラスを定義します。このクラスは PSD ファイルに埋め込まれたもの、または外部ファイルにリンクされたスマートオブジェクトを含みます。"
type: docs
weight: 10
url: /ja/java/com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/
---
...
SmartObjectLayer クラスは、PSD ファイルに埋め込まれたスマートオブジェクトまたは外部ファイルにリンクされたスマートオブジェクトを含みます。Smart Objects を使用すると、以下が可能です。破壊的でない変換を実行できます。レイヤーをスケール、回転、歪み、変形、遠近変換、またはワープさせても、元の画像データや品質を失わずに済みます（変換は元データに影響しません）don\\ufffdt。Illustrator のベクターアートワークなどのベクターデータを、ラスタライズされることなく扱えます。破壊的でないフィルタリングを実行できます。Smart Objects に適用されたフィルタをいつでも編集できます。1 つの Smart Object を編集すると、リンクされたすべてのインスタンスが自動的に更新されます。Smart Object レイヤーにリンクまたは非リンクのレイヤーマスクを適用できます（that\\ufffds）。低解像度のプレースホルダー画像でさまざまなデザインを試し、後で最終版に差し替えることができます。Adobe\\ufffd Photoshop\\ufffd では、画像の内容を PSD ドキュメントに埋め込むことができます。詳細はここにあります: 埋め込まれたスマートオブジェクトを含むレイヤーは、placed (PlLd) および SoLd リソースとスマートオブジェクトプロパティを保持します。PlLd リソースは PSD バージョン 10 未満では単独で存在できます。これらのリソースは、global Lnk2Resource 内の LiFdDataSource の UniqueId、埋め込みファイル名、その他のパラメータ、元の形式のバイト配列としての埋め込みファイル内容を含みます。
## Constructors

| Constructor | 説明 |
| --- | --- |
| [SmartObjectLayer(System.IO.Stream stream)](#SmartObjectLayer-com.aspose.ms.System.IO.Stream-) | 新しい [SmartObjectLayer](../../com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer) クラスのインスタンスを初期化します。 |
## フィールド

| フィールド | 説明 |
| --- | --- |
| [BlendSignature](#BlendSignature) | ブレンドモードシグネチャを表します。 |
| [BothPlacedAndSmartObjectResourcesFoundMessage_internalized](#BothPlacedAndSmartObjectResourcesFoundMessage-internalized) | 配置されたリソースとスマートオブジェクトリソースの両方が見つかるメッセージ |
| [CanOnlyCopyEmbeddedSmartObjectMessage_internalized](#CanOnlyCopyEmbeddedSmartObjectMessage-internalized) | メッセージ: 埋め込みスマートオブジェクトのみコピーできます |
| [LayerHeaderSize](#LayerHeaderSize) | レイヤーヘッダーのサイズです。 |
| [NoPlacedOrSmartObjectResourceFoundMessage_internalized](#NoPlacedOrSmartObjectResourceFoundMessage-internalized) | 配置またはスマートオブジェクトリソースが見つからないメッセージ |
...
| [WrongPlacedResourceIndicesMessage_internalized](#WrongPlacedResourceIndicesMessage-internalized) | 配置リソースインデックスが間違っているメッセージ |
...
| [convertToLinked(String linkedPath)](#convertToLinked-java.lang.String-) | この埋め込みスマートオブジェクトをリンクされたスマートオブジェクトに変換します。 |
...
| [duplicateLayer()](#duplicateLayer--) | このレイヤーをコピーして新しいスマートオブジェクトレイヤーを作成します。 |
| [embedLinked()](#embedLinked--) | このレイヤーにリンクされたスマートオブジェクトを埋め込みます。 |
| [equals(Object obj)](#equals-java.lang.Object-) | 指定されたオブジェクトがこのインスタンスと等しいかどうかを判定します。 |
| [exportContents(String filePath)](#exportContents-java.lang.String-) | 埋め込みまたはリンクされたコンテンツをファイルにエクスポートします。 |
...
| [getAllowWarpRepaint_internalized()](#getAllowWarpRepaint-internalized--) | レンダー ワープを取得または設定するか、PS 画像を使用します |
...
| [getContentType()](#getContentType--) | スマートオブジェクトレイヤーコンテンツのタイプを取得します。 |
| [getContents()](#getContents--) | スマートオブジェクトレイヤーのコンテンツを取得または設定します。 |
| [getContentsBounds()](#getContentsBounds--) | スマートオブジェクトコンテンツの境界を取得または設定します。 |
| [getContentsSource()](#getContentsSource--) | スマートオブジェクトコンテンツのソースを取得または設定します。 |
...
| [getImageSizeBeforeChange_internalized()](#getImageSizeBeforeChange-internalized--) | 画像をワープするための最後のサイズを取得または設定します。 |
...
| [getOriginalQuadrilateral_internalized()](#getOriginalQuadrilateral-internalized--) | 元の四辺形を取得または設定します。 |
...
| [getPlacedLayerResource_internalized(LayerResource[] resources, int[] placedResourceIndex, int[] smartResourceIndex)](#getPlacedLayerResource-internalized-com.aspose.psd.fileformats.psd.layers.LayerResource---int---int---) | 配置されたリソースとスマートレイヤーリソースのインデックス、および適切な配置リソースを取得します。 |
| [getPlacedResource_internalized()](#getPlacedResource-internalized--) | 配置レイヤーリソースを取得します。 |
...
| [getSmartFilters()](#getSmartFilters--) | スマートフィルターを取得します。 |
...
| [getTransformedQuadrilateral_internalized()](#getTransformedQuadrilateral-internalized--) | 変換四辺形を取得または設定します。 |
...
| [getWarpProvider_internalized()](#getWarpProvider-internalized--) | ワープに役立つ機能を取得します。 |
| [getWarpSettings()](#getWarpSettings--) | リソースから設定または取得された Warp パラメータを取得または設定します（デフォルト）。 |
...
| [loadContents(LoadOptions options)](#loadContents-com.aspose.psd.LoadOptions-) | スマートオブジェクトレイヤーの埋め込みまたはリンクされた画像コンテンツを取得します。 |
...
| [newSmartObjectViaCopy()](#newSmartObjectViaCopy--) | このレイヤーをコピーして新しいスマートオブジェクトレイヤーを作成します。 |
...
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [onContainerSet_internalized()](#onContainerSet-internalized--) | この[Layer](../../com.aspose.psd.fileformats.psd.layers/layer)のコンテナが設定されたときに呼び出します。 |
...
| [relinkToFile(String linkedPath)](#relinkToFile-java.lang.String-) | リンクされたスマートオブジェクトを新しいファイルに再リンクします。 |
...
| [replaceContents(Image image)](#replaceContents-com.aspose.psd.Image-) | スマートオブジェクトレイヤーに埋め込まれたスマートオブジェクトのコンテンツを置き換えます。 |
| [replaceContents(Image image, ResolutionSetting resolution)](#replaceContents-com.aspose.psd.Image-com.aspose.psd.ResolutionSetting-) | スマートオブジェクトレイヤーに埋め込まれたスマートオブジェクトのコンテンツを置き換えます。 |
| [replaceContents(String linkedPath)](#replaceContents-java.lang.String-) | コンテンツをファイルで置き換えます。 |
| [replaceContents(String linkedPath, boolean isReplaceOnlyThis)](#replaceContents-java.lang.String-boolean-) | コンテンツをファイルで置き換えます。 |
| [replaceContents(String linkedPath, ResolutionSetting resolution)](#replaceContents-java.lang.String-com.aspose.psd.ResolutionSetting-) | コンテンツをファイルで置き換えます。 |
| [replaceContents(String linkedPath, ResolutionSetting resolution, boolean isReplaceOnlyThis)](#replaceContents-java.lang.String-com.aspose.psd.ResolutionSetting-boolean-) | コンテンツをファイルで置き換えます。 |
...
| [setAllowWarpRepaint_internalized(boolean value)](#setAllowWarpRepaint-internalized-boolean-) | レンダー ワープを取得または設定するか、PS 画像を使用します |
...
| [setContents(byte[] value)](#setContents-byte---) | スマートオブジェクトレイヤーのコンテンツを取得または設定します。 |
| [setContentsBounds(Rectangle value)](#setContentsBounds-com.aspose.psd.Rectangle-) | スマートオブジェクトコンテンツの境界を取得または設定します。 |
| [setContentsSource(LinkDataSource value)](#setContentsSource-com.aspose.psd.fileformats.psd.layers.layerresources.linkresources.LinkDataSource-) | スマートオブジェクトコンテンツのソースを取得または設定します。 |
...
| [setImageSizeBeforeChange_internalized(Size value)](#setImageSizeBeforeChange-internalized-com.aspose.psd.Size-) | 画像をワープするための最後のサイズを取得または設定します。 |
...
| [setOriginalQuadrilateral_internalized(Quadrilateral value)](#setOriginalQuadrilateral-internalized-com.aspose.internal.fileformats.psd.projective.Quadrilateral-) | 元の四辺形を取得または設定します。 |
...
| [setSmartObjectProvider_internalized(SmartObjectProvider value)](#setSmartObjectProvider-internalized-com.aspose.psd.fileformats.psd.SmartObjectProvider-) | スマートオブジェクトプロバイダーを取得します。 |
| [setTop(int value)](#setTop-int-) | 最上位レイヤーの位置を取得または設定します。 |
| [setTransformedQuadrilateral_internalized(Quadrilateral value)](#setTransformedQuadrilateral-internalized-com.aspose.internal.fileformats.psd.projective.Quadrilateral-) | 変換四辺形を取得または設定します。 |
...
| [setWarpSettings(WarpSettings value)](#setWarpSettings-com.aspose.psd.fileformats.psd.layers.warp.structs.WarpSettings-) | リソースから設定または取得された Warp パラメータを取得または設定します（デフォルト）。 |
...
| [updateDataSourceId_internalized(System.Guid sourceId)](#updateDataSourceId-internalized-com.aspose.ms.System.Guid-) | データソース識別子を更新します。 |
| [updateModifiedContent()](#updateModifiedContent--) | 変更されたコンテンツでスマートオブジェクトレイヤーの画像キャッシュを更新します。 |
| [updateModifiedContentWithImage_internalized(Image image, ResolutionSetting resolution, boolean skipBoundsUpdate)](#updateModifiedContentWithImage-internalized-com.aspose.psd.Image-com.aspose.psd.ResolutionSetting-boolean-) | 変更されたコンテンツでスマートオブジェクトレイヤーの画像キャッシュを更新します。 |
| [updatePlacedResources_internalized(boolean toEmbedded)](#updatePlacedResources-internalized-boolean-) | 配置された / スマートリソースが変更されたとき、または変換が必要なときに placedResource を更新します。 |
| [updatePreview_internalized(PixelsData targetWarpedPixels)](#updatePreview-internalized-com.aspose.psd.pixelsdatamodels.PixelsData-) | "IWarpSupport" のラッパーで、レイヤープレビューを更新します。 |
| [wait()](#wait--) |  |
...
新しい [SmartObjectLayer](../../com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer) クラスのインスタンスを初期化します。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| stream | com.aspose.ms.System.IO.Stream | 項目のストリーム |
...
配置されたリソースとスマートオブジェクトリソースの両方が見つかるメッセージ
...
配置またはスマートオブジェクトリソースが見つからないメッセージ
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| linkedPath | java.lang.String | リンクされたパス。 |
...
このレイヤーをコピーして新しいスマートオブジェクトレイヤーを作成します。埋め込みスマートオブジェクトの場合、埋め込み画像は共有されます。埋め込み画像をコピーしたい場合は [.newSmartObjectViaCopy](../../null/\\#newSmartObjectViaCopy) メソッドを使用してください。
...
このレイヤーにリンクされたスマートオブジェクトを埋め込みます。
...
埋め込みまたはリンクされたコンテンツをファイルにエクスポートします。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| filePath | java.lang.String | エクスポートファイルパス。 |
...
レンダー ワープを取得または設定するか、PS 画像を使用します
...
スマートオブジェクトレイヤーコンテンツのタイプを取得します。埋め込みスマートオブジェクトの内容は、埋め込み生画像ファイルです: LiFdDataSource.Data。リンクされたスマートオブジェクトの内容は、利用可能な場合はリンク画像ファイルの生データです: [LiFeDataSource](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource)。LinkDataSource.IsLibraryLink が true のとき、Adobe\\ufffd Photoshop\\ufffd \\ufffd\\ufffd Graphics Library からの読み込みはサポートしていません（[LinkDataSource.isLibraryLink()](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\\#isLibraryLink--)/[LinkDataSource.setLibraryLink(boolean)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\\#setLibraryLink-boolean-)）。通常のリンクファイルの場合、まず LiFeDataSource.RelativePath ([LiFeDataSource.getRelativePath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#getRelativePath)/[LiFeDataSource.setRelativePath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#setRelativePath-String-)) を使用して、ソース画像パス DataStreamSupporter.SourceImagePath に対して相対的にファイルを検索します。利用できない場合は LiFeDataSource.FullPath ([LiFeDataSource.getFullPath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#getFullPath)/[LiFeDataSource.setFullPath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#setFullPath-String-)) を確認し、それでも見つからなければ画像と同じディレクトリにあるリンクファイルを探します: DataStreamSupporter.SourceImagePath。

値: スマートオブジェクトレイヤーコンテンツのタイプ。
...
スマートオブジェクトレイヤーの内容を取得または設定します。埋め込みスマートオブジェクトの内容は、埋め込み生画像ファイルです: LiFdDataSource.Data とそのプロパティです。リンクされたスマートオブジェクトの内容は、利用可能な場合はリンク画像ファイルの生データとそのプロパティです: [LiFeDataSource](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource)。LinkDataSource.IsLibraryLink が true のとき、Adobe\\ufffd Photoshop\\ufffd \\ufffd\\ufffd Graphics Library からの読み込みはサポートしていません（[LinkDataSource.isLibraryLink()](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\\#isLibraryLink--)/[LinkDataSource.setLibraryLink(boolean)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\\#setLibraryLink-boolean-)）。通常のリンクファイルの場合、まず LiFeDataSource.RelativePath ([LiFeDataSource.getRelativePath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#getRelativePath)/[LiFeDataSource.setRelativePath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#setRelativePath-String-)) を使用して、ソース画像パス DataStreamSupporter.SourceImagePath に対して相対的にファイルを検索します。利用できない場合は LiFeDataSource.FullPath ([LiFeDataSource.getFullPath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#getFullPath)/[LiFeDataSource.setFullPath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\\#setFullPath-String-)) を確認し、それでも見つからなければ画像と同じディレクトリにあるリンクファイルを探します: DataStreamSupporter.SourceImagePath。
...
スマートオブジェクトコンテンツのソースを取得または設定します。
...
配置されたリソースとスマートレイヤーリソースのインデックス、および適切な配置リソースを取得します。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| resources | [LayerResource\[\]](../../com.aspose.psd.fileformats.psd.layers/layerresource) | リソース。 |
| placedResourceIndex | int[] | 配置されたリソースのインデックス。 |
| smartResourceIndex | int[] | スマートリソースのインデックス。 |
...
リソースから設定または取得された Warp パラメータを取得または設定します（デフォルト）。
...
スマートオブジェクトレイヤーの埋め込みまたはリンクされた画像コンテンツを取得します。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| options | [LoadOptions](../../com.aspose.psd/loadoptions) | オプション。 |
...
このレイヤーをコピーして新しいスマートオブジェクトレイヤーを作成します。Adobe\\ufffd Photoshop\\ufffd の「レイヤー -> スマートオブジェクト -> コピーで新規スマートオブジェクト」機能を再現します。埋め込みスマートオブジェクトの場合のみ有効で、埋め込み画像もコピーされます。埋め込み画像を共有したい場合は [.duplicateLayer](../../null/\\#duplicateLayer) メソッドを使用してください。
...
この[Layer](../../com.aspose.psd.fileformats.psd.layers/layer)のコンテナが設定されたときに呼び出します。
...
リンクされたスマートオブジェクトを新しいファイルに再リンクします。その後 UpdateModifiedContent メソッドを呼び出す必要はありません。
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 画像です。 |
...
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 画像です。 |
| resolution | [ResolutionSetting](../../com.aspose.psd/resolutionsetting) | 解像度設定です。null の場合は画像の解像度が使用されます。 |
...
内容をファイルで置き換えます。その後 UpdateModifiedContent メソッドを呼び出す必要はありません。
...
| --- | --- | --- |
| linkedPath | java.lang.String | リンクされたパス。 |
| isReplaceOnlyThis | boolean | このフラグは、このスマートレイヤーからのコンテンツ置換か、すべてのスマートレイヤーへのコンテンツ置換を示します。 |
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| 値 | com.aspose.internal.fileformats.psd.projective.Quadrilateral |  |
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| sourceId | com.aspose.ms.System.Guid | データ ソースの識別子。 |
...
変更されたコンテンツでスマートオブジェクトレイヤーの画像キャッシュを更新します。
...
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 画像です。 |
| resolution | [ResolutionSetting](../../com.aspose.psd/resolutionsetting) | 解像度。 |
| skipBoundsUpdate | boolean | true に設定された場合、[skip bounds update]。 |
...
配置された / スマートリソースが変更されたとき、または変換が必要なときに placedResource を更新します。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| toEmbedded | boolean | true に設定された場合、必要に応じて埋め込みリソースに変換し、そうでなければリンクされたリソースに変換します。 |
...
"IWarpSupport" のラッパーで、レイヤープレビューを更新します。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| targetWarpedPixels | [PixelsData](../../com.aspose.psd.pixelsdatamodels/pixelsdata) | 新しいプレビュー画像のピクセル数とサイズ |
