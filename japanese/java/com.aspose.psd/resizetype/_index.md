...
description: "リサイズの種類を指定します。"
type: docs
weight: 91
url: /ja/java/com.aspose.psd/resizetype/
---
...
| フィールド | 説明 |
| --- | --- |
| [AdaptiveResample](#AdaptiveResample) | 加重およびブレンドされた有理関数と lanczos3 補間アルゴリズムに基づく適応アルゴリズムを使用してリサンプリングします。 |
| [Bell](#Bell) | ベル補間法 |
| [BilinearResample](#BilinearResample) | 双一次補間を使用してリサンプリングします。 |
| [CatmullRom](#CatmullRom) | Catmull-Rom キュービック補間法。 |
| [CenterToCenter](#CenterToCenter) | 新しい画像の中心は元の画像の中心と一致します。 |
| [CubicBSpline](#CubicBSpline) | CubicBSpline キュービック補間法 |
| [CubicConvolution](#CubicConvolution) | Cubic Convolution 補間法 |
| [EnumSeparatorCharArray](#EnumSeparatorCharArray) |  |
| [HighQualityResample](#HighQualityResample) | 高品質リサンプリング |
| [LanczosResample](#LanczosResample) | a=3 の lanczos アルゴリズムを使用してリサンプリングします。 |
| [LeftBottomToLeftBottom](#LeftBottomToLeftBottom) | 新しい画像の左下点は元の画像の左下点と一致します。 |
| [LeftTopToLeftTop](#LeftTopToLeftTop) | 新しい画像の左上点は元の画像の左上点と一致します。 |
| [Mitchell](#Mitchell) | Mitchell キュービック補間法 |
| [NearestNeighbourResample](#NearestNeighbourResample) | 最近傍アルゴリズムを使用してリサンプリングします。 |
| [None](#None) | リサイズ操作中にピクセルは保持されません。 |
| [RightBottomToRightBottom](#RightBottomToRightBottom) | 新しい画像の右下点は元の画像の右下点と一致します。 |
| [RightTopToRightTop](#RightTopToRightTop) | 新しい画像の右上点は元の画像の右上点と一致します。 |
| [SinC](#SinC) | Sinc（Lanczos3）キュービック補間法 |
## メソッド
...
加重およびブレンドされた有理関数と lanczos3 補間アルゴリズムに基づく適応アルゴリズムを使用してリサンプリングします。
...
双一次補間を使用してリサンプリングします。必要に応じて、リサンプリング前にノイズを除去するための画像事前フィルタリングが許可されます。
...
新しい画像の中心は元の画像の中心と一致します。必要に応じてクロップが行われます。
...
a=3 の lanczos アルゴリズムを使用してリサンプリングします。
...
新しい画像の左下点は元の画像の左下点と一致します。必要に応じてクロップが行われます。
...
新しい画像の左上の点は元の画像の左上の点と一致します。必要に応じて切り抜きが行われます。
...
新しい画像の右下の点は元の画像の右下の点と一致します。必要に応じて切り抜きが行われます。
...
新しい画像の右上の点は元の画像の右上の点と一致します。必要に応じて切り抜きが行われます。
