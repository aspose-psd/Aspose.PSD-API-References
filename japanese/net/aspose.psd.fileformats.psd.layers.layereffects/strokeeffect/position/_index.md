...
description: StrokeEffect 財産. ストローク エフェクトの位置を取得または設定してPSD レイヤー コンテンツに対するストロークの位置合わせを制御します 値は次のとおりですInside PSD レイヤー コンテンツ内にストロークを描画するには またはOutsidePSD レイヤー コンテンツの周りにストロークを描画するには とCenter内側と外側の両方にストロークを描く.
type: docs
weight: 70
