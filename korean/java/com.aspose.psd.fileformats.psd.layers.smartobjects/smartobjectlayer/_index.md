...
description: "PSD 파일에 포함된 임베디드 스마트 오브젝트 또는 외부 파일에 연결된 스마트 오브젝트를 포함하는 SmartObjectLayer 클래스를 정의합니다."
type: docs
weight: 10
url: /ko/java/com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/
---
...
SmartObjectLayer 클래스를 정의합니다. 이 클래스는 PSD 파일에 포함된 스마트 오브젝트 또는 외부 파일에 연결된 스마트 오브젝트를 포함합니다. 스마트 오브젝트를 사용하면 다음을 수행할 수 있습니다: 비파괴 변환 수행. 변환이 원본 데이터를 don\\ufffdt 영향을 주지 않기 때문에 레이어를 스케일, 회전, 기울이기, 왜곡, 원근 변환 또는 워핑하여 원본 이미지 데이터나 품질을 잃지 않습니다. 일러스트레이터의 벡터 아트워크와 같은 벡터 데이터를 작업할 수 있으며, 그렇지 않으면 래스터화됩니다. 비파괴 필터링 수행. 스마트 오브젝트에 적용된 필터를 언제든지 편집할 수 있습니다. 하나의 스마트 오브젝트를 편집하면 해당 연결된 모든 인스턴스가 자동으로 업데이트됩니다. 레이어 마스크를 적용합니다 that\\ufffds 연결되었거나 연결되지 않은 스마트 오브젝트 레이어에. 저해상도 플레이스홀더 이미지를 사용하여 다양한 디자인을 시도하고 나중에 최종 버전으로 교체할 수 있습니다. Adobe\\ufffd Photoshop\\ufffd에서 이미지 내용을 PSD 문서에 삽입할 수 있습니다. 자세한 내용은 여기에서 확인하십시오:   포함된 스마트 오브젝트가 있는 레이어는 배치된(PlLd) 및 SoLd 리소스를 포함하며 스마트 오브젝트 속성을 가집니다. PlLd 리소스는 PSD 버전 10 이전에서는 단독으로 존재할 수 있습니다. 이러한 리소스는 전역 Lnk2Resource에 있는 LiFdDataSource의 UniqueId와 포함된 파일 이름 및 기타 매개변수를 포함하며, 원본 형식의 파일 내용을 바이트 배열로 포함합니다.
## 생성자

| 생성자 | 설명 |
| --- | --- |
| [SmartObjectLayer(System.IO.Stream stream)](#SmartObjectLayer-com.aspose.ms.System.IO.Stream-) | 새로운 [SmartObjectLayer](../../com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer) 클래스의 인스턴스를 초기화합니다. |
## 필드

| 필드 | 설명 |
| --- | --- |
| [BlendSignature](#BlendSignature) | 블렌드 모드 서명을 나타냅니다. |
| [BothPlacedAndSmartObjectResourcesFoundMessage_internalized](#BothPlacedAndSmartObjectResourcesFoundMessage-internalized) | 배치된 및 스마트 객체 리소스가 모두 발견된 메시지 |
| [CanOnlyCopyEmbeddedSmartObjectMessage_internalized](#CanOnlyCopyEmbeddedSmartObjectMessage-internalized) | 임베드된 스마트 객체만 복사할 수 있다는 메시지 |
| [LayerHeaderSize](#LayerHeaderSize) | 레이어 헤더 크기입니다. |
| [NoPlacedOrSmartObjectResourceFoundMessage_internalized](#NoPlacedOrSmartObjectResourceFoundMessage-internalized) | 배치된 객체 또는 스마트 객체 리소스가 발견되지 않았다는 메시지 |
...
| [WrongPlacedResourceIndicesMessage_internalized](#WrongPlacedResourceIndicesMessage-internalized) | 잘못된 배치 리소스 인덱스에 대한 메시지 |
...
| [convertToLinked(String linkedPath)](#convertToLinked-java.lang.String-) | 이 임베드된 스마트 객체를 링크된 스마트 객체로 변환합니다. |
...
| [duplicateLayer()](#duplicateLayer--) | 이 레이어를 복사하여 새로운 스마트 객체 레이어를 생성합니다. |
| [embedLinked()](#embedLinked--) | 링크된 스마트 객체를 이 레이어에 임베드합니다. |
| [equals(Object obj)](#equals-java.lang.Object-) | 지정된 객체가 이 인스턴스와 같은지 확인합니다. |
| [exportContents(String filePath)](#exportContents-java.lang.String-) | 임베드되거나 링크된 내용을 파일로 내보냅니다. |
...
| [getAllowWarpRepaint_internalized()](#getAllowWarpRepaint-internalized--) | 렌더 워프를 가져오거나 설정하거나 PS 이미지를 사용합니다 |
...
| [getBottom()](#getBottom--) | 하단 레이어 위치를 가져오거나 설정합니다. |
...
| [getContentType()](#getContentType--) | 스마트 오브젝트 레이어 콘텐츠의 유형을 가져옵니다. |
| [getContents()](#getContents--) | 스마트 객체 레이어 내용을 가져오거나 설정합니다. |
| [getContentsBounds()](#getContentsBounds--) | 스마트 객체 내용의 경계를 가져오거나 설정합니다. |
| [getContentsSource()](#getContentsSource--) | 스마트 객체 내용의 소스를 가져오거나 설정합니다. |
...
| [getImageSizeBeforeChange_internalized()](#getImageSizeBeforeChange-internalized--) | 왜곡할 이미지의 마지막 크기를 가져오거나 설정합니다. |
...
| [getLeft()](#getLeft--) | 왼쪽 레이어 위치를 가져오거나 설정합니다. |
...
| [getOriginalQuadrilateral_internalized()](#getOriginalQuadrilateral-internalized--) | 원본 사각형을 가져오거나 설정합니다. |
...
| [getPlacedLayerResource_internalized(LayerResource[] resources, int[] placedResourceIndex, int[] smartResourceIndex)](#getPlacedLayerResource-internalized-com.aspose.psd.fileformats.psd.layers.LayerResource---int---int---) | 배치된 레이어와 스마트 레이어 리소스 인덱스 및 올바른 배치 리소스를 가져옵니다. |
| [getPlacedResource_internalized()](#getPlacedResource-internalized--) | 배치된 레이어 리소스를 가져옵니다. |
...
| [getRight()](#getRight--) | 오른쪽 레이어 위치를 가져오거나 설정합니다. |
...
| [getSmartFilters()](#getSmartFilters--) | 스마트 필터를 가져옵니다. |
...
| [getTop()](#getTop--) | 상위 레이어 위치를 가져오거나 설정합니다. |
| [getTransformedQuadrilateral_internalized()](#getTransformedQuadrilateral-internalized--) | 변환 사각형을 가져오거나 설정합니다. |
...
| [getWarpProvider_internalized()](#getWarpProvider-internalized--) | 워프와 관련된 기능을 가져옵니다 |
| [getWarpSettings()](#getWarpSettings--) | 리소스에서 설정되거나 가져온 워프 매개변수를 가져오거나 설정합니다 (기본값) |
...
| [loadContents(LoadOptions options)](#loadContents-com.aspose.psd.LoadOptions-) | 스마트 객체 레이어의 임베드되거나 링크된 이미지 내용을 가져옵니다. |
...
| [newSmartObjectViaCopy()](#newSmartObjectViaCopy--) | 이 레이어를 복사하여 새로운 스마트 객체 레이어를 생성합니다. |
...
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [onContainerSet_internalized()](#onContainerSet-internalized--) | 이 [Layer](../../com.aspose.psd.fileformats.psd.layers/layer)의 컨테이너가 설정될 때 호출됩니다. |
...
| [relinkToFile(String linkedPath)](#relinkToFile-java.lang.String-) | 링크된 스마트 객체를 새 파일에 다시 연결합니다. |
...
| [replaceContents(Image image)](#replaceContents-com.aspose.psd.Image-) | 스마트 객체 레이어에 임베드된 스마트 객체 내용을 교체합니다. |
| [replaceContents(Image image, ResolutionSetting resolution)](#replaceContents-com.aspose.psd.Image-com.aspose.psd.ResolutionSetting-) | 스마트 객체 레이어에 임베드된 스마트 객체 내용을 교체합니다. |
| [replaceContents(String linkedPath)](#replaceContents-java.lang.String-) | 내용을 파일로 교체합니다. |
| [replaceContents(String linkedPath, boolean isReplaceOnlyThis)](#replaceContents-java.lang.String-boolean-) | 내용을 파일로 교체합니다. |
| [replaceContents(String linkedPath, ResolutionSetting resolution)](#replaceContents-java.lang.String-com.aspose.psd.ResolutionSetting-) | 내용을 파일로 교체합니다. |
| [replaceContents(String linkedPath, ResolutionSetting resolution, boolean isReplaceOnlyThis)](#replaceContents-java.lang.String-com.aspose.psd.ResolutionSetting-boolean-) | 내용을 파일로 교체합니다. |
...
| [setAllowWarpRepaint_internalized(boolean value)](#setAllowWarpRepaint-internalized-boolean-) | 렌더 워프를 가져오거나 설정하거나 PS 이미지를 사용합니다 |
...
| [setBottom(int value)](#setBottom-int-) | 하단 레이어 위치를 가져오거나 설정합니다. |
...
| [setContents(byte[] value)](#setContents-byte---) | 스마트 객체 레이어 내용을 가져오거나 설정합니다. |
| [setContentsBounds(Rectangle value)](#setContentsBounds-com.aspose.psd.Rectangle-) | 스마트 객체 내용의 경계를 가져오거나 설정합니다. |
| [setContentsSource(LinkDataSource value)](#setContentsSource-com.aspose.psd.fileformats.psd.layers.layerresources.linkresources.LinkDataSource-) | 스마트 객체 내용의 소스를 가져오거나 설정합니다. |
...
| [setImageSizeBeforeChange_internalized(Size value)](#setImageSizeBeforeChange-internalized-com.aspose.psd.Size-) | 왜곡할 이미지의 마지막 크기를 가져오거나 설정합니다. |
...
| [setLeft(int value)](#setLeft-int-) | 왼쪽 레이어 위치를 가져오거나 설정합니다. |
...
| [setOriginalQuadrilateral_internalized(Quadrilateral value)](#setOriginalQuadrilateral-internalized-com.aspose.internal.fileformats.psd.projective.Quadrilateral-) | 원본 사각형을 가져오거나 설정합니다. |
...
| [setRight(int value)](#setRight-int-) | 오른쪽 레이어 위치를 가져오거나 설정합니다. |
...
| [setSmartObjectProvider_internalized(SmartObjectProvider value)](#setSmartObjectProvider-internalized-com.aspose.psd.fileformats.psd.SmartObjectProvider-) | 스마트 객체 제공자를 가져옵니다. |
| [setTop(int value)](#setTop-int-) | 상위 레이어 위치를 가져오거나 설정합니다. |
| [setTransformedQuadrilateral_internalized(Quadrilateral value)](#setTransformedQuadrilateral-internalized-com.aspose.internal.fileformats.psd.projective.Quadrilateral-) | 변환 사각형을 가져오거나 설정합니다. |
...
| [setWarpSettings(WarpSettings value)](#setWarpSettings-com.aspose.psd.fileformats.psd.layers.warp.structs.WarpSettings-) | 리소스에서 설정되거나 가져온 워프 매개변수를 가져오거나 설정합니다 (기본값) |
...
| [updateDataSourceId_internalized(System.Guid sourceId)](#updateDataSourceId-internalized-com.aspose.ms.System.Guid-) | 데이터 소스 식별자를 업데이트합니다. |
| [updateModifiedContent()](#updateModifiedContent--) | 수정된 내용으로 스마트 객체 레이어 이미지 캐시를 업데이트합니다. |
| [updateModifiedContentWithImage_internalized(Image image, ResolutionSetting resolution, boolean skipBoundsUpdate)](#updateModifiedContentWithImage-internalized-com.aspose.psd.Image-com.aspose.psd.ResolutionSetting-boolean-) | 수정된 내용으로 스마트 객체 레이어 이미지 캐시를 업데이트합니다. |
| [updatePlacedResources_internalized(boolean toEmbedded)](#updatePlacedResources-internalized-boolean-) | 배치된 / 스마트 리소스가 변경되거나 변환이 필요할 때 placedResource를 업데이트합니다. |
| [updatePreview_internalized(PixelsData targetWarpedPixels)](#updatePreview-internalized-com.aspose.psd.pixelsdatamodels.PixelsData-) | \"IWarpSupport\"에 대한 래퍼이며 레이어 미리보기를 업데이트합니다. |
| [wait()](#wait--) |  |
...
새로운 [SmartObjectLayer](../../com.aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer) 클래스의 인스턴스를 초기화합니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| stream | com.aspose.ms.System.IO.Stream | 항목들의 스트림 |
...
배치된 객체 또는 스마트 객체 리소스가 발견되지 않았다는 메시지
...
이 임베드된 스마트 객체를 링크된 스마트 객체로 변환합니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| linkedPath | java.lang.String | 연결된 경로. |
...
이 레이어를 복사하여 새로운 스마트 객체 레이어를 생성합니다. 임베디드 스마트 객체의 경우 임베디드 이미지가 공유된다는 점에 유의하십시오. 임베디드 이미지를 복사하려면 [.newSmartObjectViaCopy](../../null/\#newSmartObjectViaCopy) 메서드를 사용하세요.
...
임베드되거나 링크된 내용을 파일로 내보냅니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| filePath | java.lang.String | 내보내기 파일 경로. |
...
렌더 워프를 가져오거나 설정하거나 PS 이미지를 사용합니다
...
스마트 객체 레이어 콘텐츠의 유형을 가져옵니다. 임베디드 스마트 객체 콘텐츠는 임베디드 원시 이미지 파일인 LiFdDataSource.Data 입니다. 연결된 스마트 객체 콘텐츠는 사용 가능한 경우 연결된 이미지 파일의 원시 콘텐츠이며, [LiFeDataSource](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource) 를 통해 확인할 수 있습니다. LinkDataSource.IsLibraryLink가 true인 경우 Adobe\ufffd Photoshop\ufffd \ufffd\ufffd Graphics Library에서 로드하는 것을 지원하지 않습니다 ([LinkDataSource.isLibraryLink()](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\#isLibraryLink--)/[LinkDataSource.setLibraryLink(boolean)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\#setLibraryLink-boolean-)). 일반 링크 파일의 경우 먼저 LiFeDataSource.RelativePath ([LiFeDataSource.getRelativePath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#getRelativePath)/[LiFeDataSource.setRelativePath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#setRelativePath-String-)) 를 사용하여 소스 이미지 경로 DataStreamSupporter.SourceImagePath에 상대적으로 파일을 찾습니다. 사용 불가능하면 LiFeDataSource.FullPath ([LiFeDataSource.getFullPath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#getFullPath)/[LiFeDataSource.setFullPath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#setFullPath-String-)) 를 확인하고, 그래도 없으면 이미지와 동일한 디렉터리인 DataStreamSupporter.SourceImagePath에서 링크 파일을 찾습니다.

값: 스마트 객체 레이어 콘텐츠의 유형.
...
스마트 객체 레이어 콘텐츠를 가져오거나 설정합니다. 임베디드 스마트 객체 콘텐츠는 임베디드 원시 이미지 파일 LiFdDataSource.Data와 해당 속성입니다. 연결된 스마트 객체 콘텐츠는 사용 가능한 경우 연결된 이미지 파일의 원시 콘텐츠와 해당 속성이며, [LiFeDataSource](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource) 를 통해 확인할 수 있습니다. LinkDataSource.IsLibraryLink가 true인 경우 Adobe\ufffd Photoshop\ufffd \ufffd\ufffd Graphics Library에서 로드하는 것을 지원하지 않습니다 ([LinkDataSource.isLibraryLink()](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\#isLibraryLink--)/[LinkDataSource.setLibraryLink(boolean)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/linkdatasource\#setLibraryLink-boolean-)). 일반 링크 파일의 경우 먼저 LiFeDataSource.RelativePath ([LiFeDataSource.getRelativePath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#getRelativePath)/[LiFeDataSource.setRelativePath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#setRelativePath-String-)) 를 사용하여 소스 이미지 경로 DataStreamSupporter.SourceImagePath에 상대적으로 파일을 찾습니다. 사용 불가능하면 LiFeDataSource.FullPath ([LiFeDataSource.getFullPath](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#getFullPath)/[LiFeDataSource.setFullPath(String)](../../com.aspose.psd.fileformats.psd.layers.layerresources.linkresources/lifedatasource\#setFullPath-String-)) 를 확인하고, 그래도 없으면 이미지와 동일한 디렉터리인 DataStreamSupporter.SourceImagePath에서 링크 파일을 찾습니다.
...
배치된 레이어와 스마트 레이어 리소스 인덱스 및 올바른 배치 리소스를 가져옵니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| resources | [LayerResource\[\]](../../com.aspose.psd.fileformats.psd.layers/layerresource) | 리소스. |
| placedResourceIndex | int[] | 배치된 리소스의 인덱스. |
| smartResourceIndex | int[] | 스마트 리소스의 인덱스. |
...
리소스에서 설정되거나 가져온 워프 매개변수를 가져오거나 설정합니다 (기본값)
...
스마트 객체 레이어의 임베드되거나 링크된 이미지 내용을 가져옵니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| options | [LoadOptions](../../com.aspose.psd/loadoptions) | 옵션. |
...
이 레이어를 복사하여 새로운 스마트 객체 레이어를 생성합니다. Adobe\ufffd Photoshop\ufffd의 Layer → Smart Objects → New Smart Object via Copy 기능을 재현합니다. 임베디드 이미지도 복사되기 때문에 임베디드 스마트 객체에만 이 기능이 활성화됩니다. 임베디드 이미지를 공유하려면 [.duplicateLayer](../../null/\#duplicateLayer) 메서드를 사용하세요.
...
이 [Layer](../../com.aspose.psd.fileformats.psd.layers/layer)의 컨테이너가 설정될 때 호출됩니다.
...
연결된 스마트 객체를 새 파일에 다시 연결합니다. 이후 UpdateModifiedContent 메서드를 호출할 필요가 없습니다.
...
스마트 객체 레이어에 임베드된 스마트 객체 내용을 교체합니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 이미지입니다. |
...
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 이미지입니다. |
| resolution | [ResolutionSetting](../../com.aspose.psd/resolutionsetting) | 해상도 설정입니다. null인 경우 이미지 해상도가 사용됩니다. |
...
내용을 파일로 교체합니다. 이후 UpdateModifiedContent 메서드를 호출할 필요가 없습니다.
...
| --- | --- | --- |
| linkedPath | java.lang.String | 연결된 경로. |
| isReplaceOnlyThis | boolean | 이 플래그는 이 스마트 레이어의 내용을 교체할지, 이 내용을 가진 모든 스마트 레이어에 적용할지를 나타냅니다. |
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| 값 | com.aspose.internal.fileformats.psd.projective.Quadrilateral |  |
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| sourceId | com.aspose.ms.System.Guid | 데이터 소스 식별자. |
...
수정된 내용으로 스마트 객체 레이어 이미지 캐시를 업데이트합니다.
...
| --- | --- | --- |
| image | [Image](../../com.aspose.psd/image) | 이미지입니다. |
| resolution | [ResolutionSetting](../../com.aspose.psd/resolutionsetting) | 해상도. |
| skipBoundsUpdate | boolean | true 로 설정된 경우 [skip bounds update]. |
...
배치된 / 스마트 리소스가 변경되거나 변환이 필요할 때 placedResource를 업데이트합니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| toEmbedded | boolean | true 로 설정된 경우 필요에 따라 임베드된 리소스로 변환하고, 그렇지 않으면 연결된 리소스로 변환합니다. |
...
\"IWarpSupport\"에 대한 래퍼이며 레이어 미리보기를 업데이트합니다.

**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| targetWarpedPixels | [PixelsData](../../com.aspose.psd.pixelsdatamodels/pixelsdata) | 새 미리 보기 이미지의 픽셀 및 크기 |
