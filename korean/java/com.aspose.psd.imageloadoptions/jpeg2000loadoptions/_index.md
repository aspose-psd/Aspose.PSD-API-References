...
description: "JPEG2000 로드 옵션"
type: docs
weight: 10
url: /ko/java/com.aspose.psd.imageloadoptions/jpeg2000loadoptions/
---
...
| 생성자 | 설명 |
| --- | --- |
| [Jpeg2000LoadOptions()](#Jpeg2000LoadOptions--) | 새 인스턴스를 초기화합니다 Jpeg2000LoadOptions 클래스. |
## 필드
...
| [getDefaultMaximumDecodingTime_internalized()](#getDefaultMaximumDecodingTime-internalized--) | 기본 최대 디코딩 시간을 가져옵니다. |
| [getIgnoreAfterLoad_internalized()](#getIgnoreAfterLoad-internalized--) | 로드 후 [ignore after load] 여부를 나타내는 값을 가져옵니다. |
| [getMaximumDecodingTime()](#getMaximumDecodingTime--) | 초 단위의 최대 디코딩 시간을 가져옵니다 (이 옵션은 메모리가 매우 부족한 느린 머신에서 매우 큰 이미지(해상도 5500x6500 픽셀 이상)의 처리 중 멈춤을 방지하기 위해 사용할 수 있습니다). |
| [getMaximumDecodingTimeForTile()](#getMaximumDecodingTimeForTile--) | 타일에 대한 최대 디코딩 시간을 가져옵니다. |
...
| [setDefaultMaximumDecodingTime_internalized(int value)](#setDefaultMaximumDecodingTime-internalized-int-) | 기본 최대 디코딩 시간을 설정합니다. |
...
| [setMaximumDecodingTime(int value)](#setMaximumDecodingTime-int-) | 초 단위의 최대 디코딩 시간을 설정합니다 (이 옵션은 메모리가 매우 부족한 느린 머신에서 매우 큰 이미지(해상도 5500x6500 픽셀 이상)의 처리 중 멈춤을 방지하기 위해 사용할 수 있습니다). |
| [setMaximumDecodingTimeForTile(int value)](#setMaximumDecodingTimeForTile-int-) | 타일에 대한 최대 디코딩 시간을 설정합니다. |
...
새 인스턴스를 초기화합니다 Jpeg2000LoadOptions 클래스.
...
초 단위의 최대 디코딩 시간을 가져옵니다 (이 옵션은 메모리가 매우 부족한 느린 머신에서 매우 큰 이미지(해상도 5500x6500 픽셀 이상)의 처리 중 멈춤을 방지하기 위해 사용할 수 있습니다).
...
타일에 대한 최대 디코딩 시간을 가져옵니다.

값: 타일에 대한 최대 디코딩 시간.

**Returns:**
int - 타일에 대한 최대 디코딩 시간.
...
초 단위의 최대 디코딩 시간을 설정합니다 (이 옵션은 메모리가 매우 부족한 느린 머신에서 매우 큰 이미지(해상도 5500x6500 픽셀 이상)의 처리 중 멈춤을 방지하기 위해 사용할 수 있습니다).
