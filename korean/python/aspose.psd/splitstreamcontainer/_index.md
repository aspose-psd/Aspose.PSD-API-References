---
title: "SplitStreamContainer 클래스"
type: docs
weight: 4220
url: /ko/python-net/aspose.psd/splitstreamcontainer/
---
...
| **Name** | **설명** |
| :- | :- |
| [SplitStreamContainer(stream)](#SplitStreamContainer_stream_1) | 새 인스턴스를 초기화합니다 [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/) 클래스의. |
| [SplitStreamContainer(stream, dispose_stream)](#SplitStreamContainer_stream_dispose_stream_2) | 새 인스턴스를 초기화합니다 [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/) 클래스의. |
| [SplitStreamContainer(stream, dispose_stream)](#SplitStreamContainer_stream_dispose_stream_3) | 새 인스턴스를 초기화합니다 [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/) 클래스의. |
## **Properties**
| **Name** | **Type** | **Access** | **설명** |
| :- | :- | :- | :- |
...
| [insert(position, stream, dispose_stream)](#insert_position_stream_dispose_stream_1) | 스트림 컨테이너를 지정된 위치에 삽입합니다. |
| [read(buffer, offset, count)](#read_buffer_offset_count_2) | 현재 스트림에서 바이트 시퀀스를 읽고 읽은 바이트 수만큼 스트림 내 위치를 이동합니다. |
| [read(bytes)](#read_bytes_3) | 지정된 바이트 버퍼를 채우기 위해 바이트를 읽습니다. |
| [read_byte()](#read_byte__4) | 스트림에서 바이트를 읽고 스트림 내 위치를 한 바이트만큼 이동합니다. 스트림 끝에 도달하면 -1을 반환합니다. |
| [save(destination_stream)](#save_destination_stream_5) | 스트림의 데이터를 지정된 스트림에 저장(복사)합니다. 기본 버퍼 크기 [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/)와 스트림 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 값을 사용합니다. |
| [save(destination_stream, buffer_size)](#save_destination_stream_buffer_size_6) | 스트림의 모든 데이터를 지정된 스트림에 저장(복사)합니다. 스트림 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 값을 사용합니다. |
| [save(destination_stream, buffer_size, length)](#save_destination_stream_buffer_size_length_7) | 스트림의 데이터를 지정된 스트림에 저장(복사)합니다. |
| [save(file_path)](#save_file_path_8) | 스트림의 데이터를 지정된 스트림에 저장(복사)합니다. 기본 버퍼 크기 [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/)와 스트림 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 값을 사용합니다. |
| [save(file_path, buffer_size)](#save_file_path_buffer_size_9) | 지정된 스트림에 스트림의 데이터를 저장(복사)합니다. 스트림 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 값을 사용합니다. |
| [save(file_path, buffer_size, length)](#save_file_path_buffer_size_length_10) | 스트림의 데이터를 지정된 스트림에 저장(복사)합니다. |
| [seek(offset, origin)](#seek_offset_origin_11) | 현재 스트림 내의 위치를 설정합니다. |
| seek_begin() | 스트림 위치를 스트림의 시작으로 설정합니다. 이 값은 StreamContainer 생성자에 전달된 시작 스트림 위치로부터의 오프셋을 나타냅니다. |
| [to_bytes()](#to_bytes__12) | 스트림 데이터를 int 배열로 변환합니다. |
| [to_bytes(position, bytes_count)](#to_bytes_position_bytes_count_13) | 스트림 데이터를 int 배열로 변환합니다. |
| [write(buffer, offset, count)](#write_buffer_offset_count_14) | 바이트 시퀀스를 현재 스트림에 쓰고, 쓰여진 바이트 수만큼 스트림 내 현재 위치를 앞으로 이동합니다. |
| [write(bytes)](#write_bytes_15) | 지정된 모든 바이트를 스트림에 씁니다. |
| [write_byte(value)](#write_byte_value_16) | 스트림의 현재 위치에 바이트를 쓰고, 스트림 내 위치를 한 바이트 앞으로 이동합니다. |
| [write_to(stream_container)](#write_to_stream_container_17) | 포함된 데이터를 다른 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/)에 복사합니다. |
| [write_to(stream_container, length)](#write_to_stream_container_length_18) | 포함된 데이터를 다른 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/)에 복사합니다. |
...
새 인스턴스를 초기화합니다 [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/) 클래스의.
...
| 매개변수 | 유형 | 설명 |
| :- | :- | :- |
| stream | [StreamContainer](/psd/python-net/aspose.psd/streamcontainer) | 데이터 스트림입니다. |
| dispose_stream | bool | <c>true</c> 로 설정하면 컨테이너가 해제될 때 스트림이 해제됩니다. |
...
| :- | :- | :- |
| position | int | 삽입할 위치. |
| stream | [StreamContainer](/psd/python-net/aspose.psd/streamcontainer) | 삽입할 스트림 컨테이너. |
| dispose_stream | bool | <c>true</c> 로 설정하면 스트림을 해제합니다. |
...
| :- | :- | :- |
| destination_stream | _io.BufferedRandom | 데이터를 저장할 스트림입니다. |
| buffer_size | int | 버퍼 크기입니다. 기본적으로 ReadWriteBytesCount 값이 사용됩니다. |
| length | long | 복사할 스트림 데이터 길이입니다. 기본적으로 길이는 [SplitStreamContainer.length](/psd/python-net/aspose.psd/splitstreamcontainer/) 값으로 설정됩니다. |
