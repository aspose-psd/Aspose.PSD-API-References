...
description: SmartObjectLayer 방법. 스마트 개체 레이어에 포함된 스마트 개체 콘텐츠를 대체합니다.
type: docs
weight: 140
...
스마트 개체 레이어에 포함된 스마트 개체 콘텐츠를 대체합니다.
...
| --- | --- | --- |
| image | Image | 이미지. |
| resolution | ResolutionSetting | 해상도 설정입니다. null이면 이미지 해상도가 사용됩니다. |

### 예외

| 예외 | 상태 |
| --- | --- |
| [PsdImageException](../../../aspose.psd.coreexceptions.imageformats/psdimageexception/) | 포함된 스마트 개체만 교체할 수 있습니다. |

### 예

이 예제는 새 콘텐츠 파일의 해상도가 다른 경우 ReplaceContents 메서드가 올바르게 작동함을 보여줍니다.
...
string filePath = baseFolder + fileName; // 원본 PSD 이미지
string newContentPath = baseFolder + "image.jpg"; // 스마트 개체에 대한 새 콘텐츠 파일
string outputFilePath = outputFolder + "ChangedPsd";
string pngOutputPath = outputFilePath + ".png"; // 출력 PNG 파일
string psdOutputPath = outputFilePath + ".psd"; // 출력 PSD 파일
...
내용을 파일로 바꿉니다. 나중에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다.
