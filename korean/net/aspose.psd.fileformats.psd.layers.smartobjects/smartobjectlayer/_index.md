---
title: "클래스 SmartObjectLayer"
second_title: "Aspose.PSD for .NET API 레퍼런스"
description: "Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectLayer 클래스. PSD 파일에 포함되었거나 외부 파일에 연결된 스마트 객체를 포함하는 SmartObjectLayer 클래스를 정의합니다. Smart Objects를 사용하면 비파괴 변환을 수행할 수 있습니다. 레이어를 원본 이미지 데이터나 품질을 잃지 않고 확대·회전·왜곡·투시 변환·워프 등을 할 수 있습니다. Illustrator의 벡터 아트워크와 같은 벡터 데이터를 래스터화되지 않게 작업할 수 있습니다. 비파괴 필터링을 수행합니다. Smart Objects에 적용된 필터를 언제든지 편집할 수 있습니다. 하나의 Smart Object를 편집하면 해당 객체와 연결된 모든 인스턴스가 자동으로 업데이트됩니다. Smart Object 레이어에 연결되거나 연결 해제된 레이어 마스크를 적용합니다. 나중에 최종 버전으로 교체할 저해상도 플레이스홀더 이미지를 사용해 다양한 디자인을 시도할 수 있습니다. Adobe Photoshop에서는 이미지를 PSD 문서에 삽입할 수 있습니다. 자세한 내용은 https//helpx.adobe.com/photoshop/using/createsmartobjects.html 를 참고하십시오. 임베드된 스마트 객체가 포함된 레이어는 PlLd 및 SoLd 리소스를 포함하며 스마트 객체 속성을 가지고 있습니다. PSD 버전 10 이전에서는 PlLd 리소스만 존재할 수 있습니다. 이러한 리소스는 전역 Lnk2Resource의 LiFdDataSource 고유 ID와 임베드된 파일 이름 및 원본 형식의 바이트 배열 형태 파일 내용을 포함합니다."
type: docs
weight: 3910
...
SmartObjectLayer 클래스를 정의합니다. 이 클래스는 PSD 파일에 포함된 스마트 오브젝트 또는 외부 파일에 연결된 스마트 오브젝트를 포함합니다. Smart Objects를 사용하면 다음을 수행할 수 있습니다: 비파괴 변환 수행. 레이어를 스케일, 회전, 기울이기, 왜곡, 원근 변환 또는 워핑할 수 있으며, 변환이 원본 데이터에 영향을 주지 않아 원본 이미지 데이터나 품질이 손실되지 않습니다. Illustrator와 같은 벡터 아트워크와 같은 벡터 데이터를 작업할 수 있으며, 그렇지 않으면 래스터화됩니다. 비파괴 필터링 수행. Smart Objects에 적용된 필터를 언제든지 편집할 수 있습니다. 하나의 Smart Object를 편집하면 연결된 모든 인스턴스가 자동으로 업데이트됩니다. Smart Object 레이어에 연결되거나 연결 해제된 레이어 마스크를 적용합니다. 나중에 최종 버전으로 교체할 저해상도 플레이스홀더 이미지를 사용해 다양한 디자인을 시도할 수 있습니다. Adobe Photoshop에서는 이미지를 PSD 문서에 포함시킬 수 있습니다. 자세한 내용은 다음을 참조하십시오: [https://helpx.adobe.com/photoshop/using/create-smart-objects.html](https://helpx.adobe.com/photoshop/using/create-smart-objects.html) 임베드된 스마트 오브젝트가 있는 레이어는 placed (PlLd) 및 SoLd 리소스를 포함하며, 스마트 오브젝트 속성을 가지고 있습니다. PSD 버전 10 이전에서는 PlLd 리소스만 존재할 수 있습니다. 이러한 리소스는 전역 Lnk2Resource에 있는 LiFdDataSource의 UniqueId와 임베드된 파일 이름 및 기타 매개변수를 포함하며, 원본 형식의 파일 내용을 바이트 배열로 포함합니다.
...
| 이름 | 설명 |
| --- | --- |
| [SmartObjectLayer](smartobjectlayer/)(Stream) | `SmartObjectLayer` 클래스의 새 인스턴스를 초기화합니다. |
...
| [Contents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/contents/) { get; set; } | 스마트 오브젝트 레이어 콘텐츠를 가져오거나 설정합니다. 임베디드 스마트 오브젝트 콘텐츠는 임베디드 원시 이미지 파일인 [`Data`](../../aspose.psd.fileformats.psd.layers.layerresources/lifddatasource/data/)과 해당 속성입니다. 링크된 스마트 오브젝트 콘텐츠는 사용 가능한 경우 링크된 이미지 파일의 원시 콘텐츠와 해당 속성인 [`LiFeDataSource`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/)입니다. [`IsLibraryLink`](../../aspose.psd.fileformats.psd.layers.layerresources/linkdatasource/islibrarylink/)가 true인 경우 Adobe Photoshop Graphics Library에서 로드를 지원하지 않습니다. 일반 링크 파일의 경우, 먼저 [`RelativePath`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/relativepath/)을 사용하여 소스 이미지 경로 SourceImagePath에 상대적으로 파일을 찾으며, 사용 불가능하면 [`FullPath`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/fullpath/)을 확인하고, 그래도 없으면 이미지와 동일한 디렉터리인 SourceImagePath에서 링크 파일을 찾습니다. |
| [ContentsBounds](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/contentsbounds/) { get; set; } | 스마트 오브젝트 콘텐츠의 경계를 가져오거나 설정합니다. |
| [ContentsSource](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/contentssource/) { get; set; } | 스마트 오브젝트 콘텐츠의 소스를 가져오거나 설정합니다. |
| [ContentType](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/contenttype/) { get; } | 스마트 오브젝트 레이어 콘텐츠의 유형을 가져옵니다. 임베디드 스마트 오브젝트 콘텐츠는 임베디드 원시 이미지 파일인 [`Data`](../../aspose.psd.fileformats.psd.layers.layerresources/lifddatasource/data/)입니다. 링크된 스마트 오브젝트 콘텐츠는 사용 가능한 경우 링크된 이미지 파일의 원시 콘텐츠인 [`LiFeDataSource`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/)입니다. [`IsLibraryLink`](../../aspose.psd.fileformats.psd.layers.layerresources/linkdatasource/islibrarylink/)가 true인 경우 Adobe Photoshop Graphics Library에서 로드를 지원하지 않습니다. 일반 링크 파일의 경우, 먼저 [`RelativePath`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/relativepath/)을 사용하여 소스 이미지 경로 SourceImagePath에 상대적으로 파일을 찾으며, 사용 불가능하면 [`FullPath`](../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/fullpath/)을 확인하고, 그래도 없으면 이미지와 동일한 디렉터리인 SourceImagePath에서 링크 파일을 찾습니다. |
...
| [SmartObjectProvider](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/smartobjectprovider/) { get; } | 스마트 객체 제공자를 가져옵니다. |
...
| [WarpSettings](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/warpsettings/) { get; set; } | 리소스에서 설정되었거나 가져온 워프 매개변수를 가져오거나 설정합니다 (기본값). |
...
| [ConvertToLinked](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/converttolinked/)(string) | 이 포함된 스마트 객체를 연결된 스마트 객체로 변환합니다. |
...
| [DuplicateLayer](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/duplicatelayer/)() | 이 레이어를 복사하여 새로운 스마트 객체 레이어를 생성합니다. 포함된 스마트 객체의 경우 포함된 이미지가 공유된다는 점에 유의하십시오. 포함된 이미지를 복사하려면 [`NewSmartObjectViaCopy`](./newsmartobjectviacopy/) 메서드를 사용하십시오. |
| [EmbedLinked](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/embedlinked/)() | 연결된 스마트 객체를 이 레이어에 포함합니다. |
| [ExportContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/exportcontents/)(string) | 포함되거나 연결된 내용을 파일로 내보냅니다. |
...
| [LoadContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/loadcontents/)(LoadOptions) | 스마트 오브젝트 레이어의 임베드되거나 링크된 이미지 내용을 가져옵니다. |
...
| virtual [MergeLayerTo](../../aspose.psd.fileformats.psd.layers/layer/mergelayerto/)(Layer) | 레이어를 지정된 레이어에 병합합니다. |
| [NewSmartObjectViaCopy](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/newsmartobjectviacopy/)() | 이 레이어를 복사하여 새로운 스마트 오브젝트 레이어를 생성합니다. Adobe Photoshop의 `Layer -&gt; Smart Objects -&gt; New Smart Object via Copy` 기능을 재현합니다. 임베드된 스마트 오브젝트에 대해서만 사용 가능하며, 임베드된 이미지도 복사됩니다. 임베드된 이미지를 공유하려면 [`DuplicateLayer`](./duplicatelayer/) 메서드를 사용하십시오. |
...
| [RelinkToFile](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/relinktofile/)(string) | 링크된 스마트 오브젝트를 새 파일에 다시 연결합니다. 이후에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다. |
...
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents)(Image) | 스마트 오브젝트 레이어에 임베드된 스마트 오브젝트 내용을 교체합니다. |
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents_2)(string) | 내용을 파일로 교체합니다. 이후에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다. |
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents_1)(Image, ResolutionSetting) | 스마트 오브젝트 레이어에 임베드된 스마트 오브젝트 내용을 교체합니다. |
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents_5)(string, bool) | 내용을 파일로 교체합니다. 이후에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다. |
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents_3)(string, ResolutionSetting) | 내용을 파일로 교체합니다. 이후에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다. |
| [ReplaceContents](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/replacecontents/#replacecontents_4)(string, ResolutionSetting, bool) | 내용을 파일로 교체합니다. 이후에 UpdateModifiedContent 메서드를 호출할 필요가 없습니다. |
...
| [UpdateModifiedContent](../../aspose.psd.fileformats.psd.layers.smartobjects/smartobjectlayer/updatemodifiedcontent/)() | 수정된 내용으로 스마트 객체 레이어 이미지 캐시를 업데이트합니다. |
