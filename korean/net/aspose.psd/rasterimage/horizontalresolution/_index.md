---
title: "RasterImage.HorizontalResolution"
second_title: "Aspose.PSD for .NET API 레퍼런스"
description: "RasterImage 속성. 이 RasterImage의 인치당 픽셀 수로 가로 해상도를 가져오거나 설정합니다."
type: docs
weight: 30
url: /ko/net/aspose.psd/rasterimage/horizontalresolution/
---
{{< psd/tize >}}
## RasterImage.HorizontalResolution property

이 [`RasterImage`](../)의 인치당 픽셀 수로 가로 해상도를 가져오거나 설정합니다.

```csharp
public virtual double HorizontalResolution { get; set; }
```

### Property Value

가로 해상도.

## 비고

참고: 기본값은 항상 96이며, 다양한 플랫폼이 화면 해상도를 반환할 수 없기 때문입니다. 두 해상도 값을 한 번에 업데이트하려면 SetResolution 메서드 사용을 고려하십시오.

### 또 보기

* class [RasterImage](../)
* namespace [Aspose.PSD](../../../aspose.psd/)
* assembly [Aspose.PSD](../../../)


