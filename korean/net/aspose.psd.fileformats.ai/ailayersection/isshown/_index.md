...
description: AiLayerSection 재산. 이 레이어가 표시되는지 여부를 나타내는 값을 가져오거나 설정합니다. true인 경우 아트보드의 레이어에 포함된 모든 아트워크를 표시합니다.
type: docs
weight: 90
url: /ko/net/aspose.psd.fileformats.ai/ailayersection/isshown/
---
## AiLayerSection.IsShown property

이 레이어가 표시되는지 여부를 나타내는 값을 가져오거나 설정합니다. true인 경우 아트보드의 레이어에 포함된 모든 아트워크를 표시합니다.
