...
description: "AiLayerSection 속성. 이 레이어가 어두워졌는지 여부를 나타내는 값을 가져오거나 설정합니다. 레이어에 포함된 연결된 이미지와 비트맵 이미지의 강도를 감소시킵니다"
type: docs
weight: 70
url: /ko/net/aspose.psd.fileformats.ai/ailayersection/isimagesdimmed/
---
{{< psd/tize >}}
## AiLayerSection.IsImagesDimmed property

이 레이어가 디밍되었는지 여부를 나타내는 값을 가져오거나 설정합니다. 레이어에 포함된 연결된 이미지와 비트맵 이미지의 강도를 감소시킵니다.
...
`true` 이 레이어가 어두워진 경우; 그렇지 않으면 `false`.
