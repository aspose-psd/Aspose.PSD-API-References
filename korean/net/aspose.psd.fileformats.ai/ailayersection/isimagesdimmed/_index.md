...
description: AiLayerSection 재산. 이 레이어가 흐리게 표시되는지 여부를 나타내는 값을 가져오거나 설정합니다. 레이어에 포함된 연결된 이미지와 비트맵 이미지의 강도를 줄입니다.
type: docs
weight: 50
...
이 레이어가 흐리게 표시되는지 여부를 나타내는 값을 가져오거나 설정합니다. 레이어에 포함된 연결된 이미지와 비트맵 이미지의 강도를 줄입니다.
