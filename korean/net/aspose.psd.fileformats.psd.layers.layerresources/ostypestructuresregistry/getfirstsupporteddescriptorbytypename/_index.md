---
title: "OSTypeStructuresRegistry.GetFirstSupportedDescriptorByTypeName"
second_title: "Aspose.PSD for .NET API 레퍼런스"
description: "OSTypeStructuresRegistry 메서드. 유형 이름으로 첫 번째 지원되는 디스크립터를 가져옵니다"
type: docs
weight: 30
url: /ko/net/aspose.psd.fileformats.psd.layers.layerresources/ostypestructuresregistry/getfirstsupporteddescriptorbytypename/
---
{{< psd/tize >}}
## OSTypeStructuresRegistry.GetFirstSupportedDescriptorByTypeName method

유형 이름으로 첫 번째 지원되는 디스크립터를 가져옵니다.

```csharp
public static IOSTypeStructureLoader GetFirstSupportedDescriptorByTypeName(
    string descriptorTypeName)
```

| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| descriptorTypeName | String | 디스크립터 유형 이름. |

### 반환 값

첫 번째로 발견된 오프너 디스크립터 또는 해당 디스크립터가 없으면 null을 반환합니다.

## 비고

첫 번째 오프너 디스크립터는 실제로 마지막에 등록된 것이 됩니다.

### 또 보기

* interface [IOSTypeStructureLoader](../../iostypestructureloader/)
* class [OSTypeStructuresRegistry](../)
* namespace [Aspose.PSD.FileFormats.Psd.Layers.LayerResources](../../../aspose.psd.fileformats.psd.layers.layerresources/)
* assembly [Aspose.PSD](../../../)


