---
title: BritResource
second_title: Aspose.PSD für .NET-API-Referenz
description: Initialisiert eine neue Instanz vonBritResourceaspose.psd.fileformats.psd.layers.layerresources/britresource Klasse.
type: docs
weight: 10
url: /de/net/aspose.psd.fileformats.psd.layers.layerresources/britresource/britresource/
---
## BritResource() {#constructor}

Initialisiert eine neue Instanz von[`BritResource`](../../britresource) Klasse.

```csharp
public BritResource()
```

### Siehe auch

* class [BritResource](../../britresource)
* namensraum [Aspose.PSD.FileFormats.Psd.Layers.LayerResources](../../britresource)
* Montage [Aspose.PSD](../../../)

---

## BritResource(short, short, short, bool) {#constructor_2}

Initialisiert eine neue Instanz von[`BritResource`](../../britresource) Klasse.

```csharp
public BritResource(short brightness, short contrast, short meanValueForBrightnessAndContrast, 
    bool labColor)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| brightness | Int16 | Die Helligkeit. |
| contrast | Int16 | Der Kontrast. |
| meanValueForBrightnessAndContrast | Int16 | Der Mittelwert für Helligkeit und Kontrast. |
| labColor | Boolean | wenn eingestellt`Stimmt` [Laborfarbe]. |

### Siehe auch

* class [BritResource](../../britresource)
* namensraum [Aspose.PSD.FileFormats.Psd.Layers.LayerResources](../../britresource)
* Montage [Aspose.PSD](../../../)

---

## BritResource(byte[]) {#constructor_1}

Initialisiert eine neue Instanz von[`BritResource`](../../britresource)class. Die PSD-Formatspezifikation enthält folgende Beschreibung: 2 Helligkeit 2 Kontrast 2 Mittelwert für Helligkeit und Kontrast 1 Nur Lab-Farbe Wird in modernen PSDs (CS5 und höher) nicht verwendet, wo CgEd ist. CgEd speichert Informationen properties

```csharp
public BritResource(byte[] bytes)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| bytes | Byte[] | Die Bytes. |

### Siehe auch

* class [BritResource](../../britresource)
* namensraum [Aspose.PSD.FileFormats.Psd.Layers.LayerResources](../../britresource)
* Montage [Aspose.PSD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
