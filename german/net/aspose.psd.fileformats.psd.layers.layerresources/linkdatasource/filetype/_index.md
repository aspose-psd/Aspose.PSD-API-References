---
title: FileType
second_title: Aspose.PSD für .NET-API-Referenz
description: Ermittelt oder setzt den Typ der eingebetteten oder externen Datei die die Adobe Photoshop Lnk2/LnkERessource enthält oder verlinkt.
type: docs
weight: 60
url: /de/net/aspose.psd.fileformats.psd.layers.layerresources/linkdatasource/filetype/
---
## LinkDataSource.FileType property

Ermittelt oder setzt den Typ der eingebetteten oder externen Datei, die die Adobe® Photoshop® Lnk2/LnkE-Ressource enthält oder verlinkt.

```csharp
public string FileType { get; set; }
```

### Eigentumswert

Der Typ der eingebetteten oder extern verlinkten Datei.

### Ausnahmen

| Ausnahme | Bedingung |
| --- | --- |
| [PsdImageException](../../../aspose.psd.coreexceptions.imageformats/psdimageexception) | Sie können nicht auf die FileType-Eigenschaft zugreifen. |

### Siehe auch

* class [LinkDataSource](../../linkdatasource)
* namensraum [Aspose.PSD.FileFormats.Psd.Layers.LayerResources](../../linkdatasource)
* Montage [Aspose.PSD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
