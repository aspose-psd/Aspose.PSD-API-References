---
title: "Κλάση JpegExifData"
type: docs
weight: 20
url: /el/python-net/aspose.psd.exif/jpegexifdata/
---
...
| **Name** | **Περιγραφή** |
| :- | :- |
| [JpegExifData()](#JpegExifData__1) | Αρχικοποιεί ένα νέο αντικείμενο της κλάσης [JpegExifData](/psd/python-net/aspose.psd.exif/jpegexifdata/). |
| [JpegExifData(common_tags, exif_tags, gps_tags)](#JpegExifData_common_tags_exif_tags_gps_tags_2) | Αρχικοποιεί ένα νέο αντικείμενο της κλάσης [JpegExifData](/psd/python-net/aspose.psd.exif/jpegexifdata/) με δεδομένα από τον πίνακα. |
| [JpegExifData(exifdata)](#JpegExifData_exifdata_3) | Αρχικοποιεί ένα νέο αντικείμενο της κλάσης [JpegExifData](/psd/python-net/aspose.psd.exif/jpegexifdata/) με δεδομένα από τον πίνακα. |
## **Properties**
| **Name** | **Type** | **Access** | **Περιγραφή** |
| :- | :- | :- | :- |
| MAX_EXIF_SEGMENT_SIZE [static] | int | r | Το μέγιστο μέγεθος τμήματος EXIF σε bytes που επιτρέπεται. |
...
| compression | ushort | r/w | Λαμβάνει ή ορίζει τη συμπίεση. |
...
| date_time | string | r/w | Λαμβάνει ή ορίζει την ημερομηνία και ώρα. |
...
| make | string | r/w | Λαμβάνει ή ορίζει τον κατασκευαστή του εξοπλισμού καταγραφής. |
...
| model | string | r/w | Λαμβάνει ή ορίζει το μοντέλο. |
| oecf | byte | r/w | Λαμβάνει ή ορίζει τη λειτουργία Οπτοηλεκτρικής Μετατροπής (OECF) που ορίζεται στο ISO 14524. |
| orientation | [ExifOrientation](/psd/python-net/aspose.psd.exif.enums/exiforientation/) | r/w | Λαμβάνει ή ορίζει τον προσανατολισμό. |
| photographic_sensitivity | uint | r/w | Λαμβάνει ή ορίζει την φωτογραφική ευαισθησία. |
| photometric_interpretation | ushort | r/w | Λαμβάνει ή ορίζει την φωτομετρική ερμηνεία. |
...
| planar_configuration | ushort | r/w | Λαμβάνει ή ορίζει τη διαμόρφωση επιπέδου. |
| primary_chromaticities | [TiffRational[]](/psd/python-net/aspose.psd.fileformats.tiff/tiffrational/) | r/w | Λαμβάνει ή ορίζει τη χρωματική απόδοση των τριών βασικών χρωμάτων της εικόνας. |
...
| reference_black_white | [TiffRational[]](/psd/python-net/aspose.psd.fileformats.tiff/tiffrational/) | r/w | Λαμβάνει ή ορίζει την αναφορά μαύρου-λευκού. |
| related_sound_file | string | r/w | Λαμβάνει ή ορίζει το σχετικό αρχείο ήχου. |
| resolution_unit | [ExifUnit](/psd/python-net/aspose.psd.exif.enums/exifunit/) | r/w | Λαμβάνει ή ορίζει τη μονάδα ανάλυσης. |
| samples_per_pixel | ushort | r/w | Λαμβάνει ή ορίζει τα δείγματα ανά pixel. |
...
| software | string | r/w | Λαμβάνει ή ορίζει το λογισμικό. |
...
| thumbnail | [RasterImage](/psd/python-net/aspose.psd/rasterimage) | r/w | Λαμβάνει ή ορίζει την μικρογραφία της εικόνας. |
| transfer_function | ushort | r/w | Λαμβάνει ή ορίζει τη συνάρτηση μεταφοράς. |
...
| x_resolution | [TiffRational](/psd/python-net/aspose.psd.fileformats.tiff/tiffrational/) | r/w | Λαμβάνει ή ορίζει την ανάλυση x. |
| y_cb_cr_coefficients | [TiffRational[]](/psd/python-net/aspose.psd.fileformats.tiff/tiffrational/) | r/w | Λαμβάνει ή ορίζει τους συντελεστές του πίνακα για τη μετατροπή από δεδομένα εικόνας RGB σε YCbCr. |
| y_cb_cr_positioning | [ExifYCbCrPositioning](/psd/python-net/aspose.psd.exif.enums/exifycbcrpositioning/) | r/w | Λαμβάνει ή ορίζει τη θέση των συνιστωσών χρωματικότητας σε σχέση με τη συνιστώσα φωτεινότητας. |
| y_cb_cr_sub_sampling | ushort | r/w | Λαμβάνει ή ορίζει το λόγο δειγματοληψίας των συνιστωσών χρωματικότητας σε σχέση με τη συνιστώσα φωτεινότητας. |
| y_resolution | [TiffRational](/psd/python-net/aspose.psd.fileformats.tiff/tiffrational/) | r/w | Λαμβάνει ή ορίζει την ανάλυση y. |
## **Methods**
| **Name** | **Περιγραφή** |
| :- | :- |
...
| [serialize_exif_data()](#serialize_exif_data__3) | Διεξάγει σειριοποίηση των δεδομένων EXIF. Γράφει τις τιμές των ετικετών και τα περιεχόμενα. Η ετικέτα μεγέθους που επηρεάζει περισσότερο είναι τα περιεχόμενα της ετικέτας μικρογραφίας. |
...
Αρχικοποιεί ένα νέο αντικείμενο της κλάσης [JpegExifData](/psd/python-net/aspose.psd.exif/jpegexifdata/).
...
Αρχικοποιεί ένα νέο αντικείμενο της κλάσης [JpegExifData](/psd/python-net/aspose.psd.exif/jpegexifdata/) με δεδομένα από τον πίνακα.
...
Διεξάγει σειριοποίηση των δεδομένων EXIF. Γράφει τις τιμές των ετικετών και τα περιεχόμενα. Η ετικέτα μεγέθους που επηρεάζει περισσότερο είναι τα περιεχόμενα της ετικέτας μικρογραφίας.
