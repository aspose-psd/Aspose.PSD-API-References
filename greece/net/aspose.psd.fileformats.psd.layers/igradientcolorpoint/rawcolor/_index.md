---
title: IGradientColorPoint.RawColor
second_title: Aspose.PSD για Αναφορά API .NET
description: IGradientColorPoint ιδιοκτησία. Παίρνει ή ορίζει το χρώμα του ακατέργαστου.
type: docs
weight: 30
url: /el/net/aspose.psd.fileformats.psd.layers/igradientcolorpoint/rawcolor/
---
## IGradientColorPoint.RawColor property

Παίρνει ή ορίζει το χρώμα του ακατέργαστου.

```csharp
public    RawColor { get; set; }
```

### Αξία περιουσίας

Το χρώμα του ακατέργαστου.

### Δείτε επίσης

* interface [IGradientColorPoint](../)
* χώρος ονομάτων [Aspose.PSD.FileFormats.Psd.Layers](../../igradientcolorpoint/)
* συνέλευση [Aspose.PSD](../../../)


