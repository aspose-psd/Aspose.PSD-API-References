...
| [FinalizeSection](../../aspose.psd.fileformats.ai/aiimage/finalizesection) { get; } | 获取完成部分。 |
| virtual [HasBackgroundColor](../../aspose.psd/image/hasbackgroundcolor) { get; set; } | 获取或设置图像是否有背景色的值。 |
| [Header](../../aspose.psd.fileformats.ai/aiimage/header) { get; } | 获取标头。 |
...
| override [IsCached](../../aspose.psd.fileformats.ai/aiimage/iscached) { get; } | 获取一个值，表示对象的数据当前是否被缓存，不需要读取数据。 |
...
| 姓名 | 描述 |
| --- | --- |
| [AddLayer](../../aspose.psd.fileformats.ai/aiimage/addlayer)(AiLayerSection) | 添加AI层部分。 |
...
| virtual [Save](../../aspose.psd/image/save)(Stream, ImageOptionsBase, Rectangle) | 根据保存选项将图像的数据以指定的文件格式保存到指定的流中。 |
...
以下示例演示了对将 Ai 格式导出为 PSD、PNG、JPG、GIF 和 TIF 格式的支持。
