...
description: "SmartObjectLayer 属性。获取智能对象图层内容的类型。嵌入的智能对象内容是嵌入的原始图像文件 Data。链接的智能对象内容是链接的图像文件的原始内容（如果可用）LiFeDataSource。我们不支持在 IsLibraryLink 为 true 时从 Adobe Photoshop 图形库加载。对于常规链接文件，首先使用 RelativePath 相对于源图像路径 SourceImagePath 查找文件，如果不可用则查找 FullPath，如果仍不可用则在与我们的图像相同的目录（SourceImagePath）中查找链接文件。"
type: docs
weight: 50
...
获取智能对象图层内容的类型。嵌入的智能对象内容是嵌入的原始图像文件：[`Data`](../../../aspose.psd.fileformats.psd.layers.layerresources/lifddatasource/data/)。链接的智能对象内容是链接的图像文件的原始内容（如果可用）：[`LiFeDataSource`](../../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/)。当 [`IsLibraryLink`](../../../aspose.psd.fileformats.psd.layers.layerresources/linkdatasource/islibrarylink/) 为 true 时，我们不支持从 Adobe Photoshop 图形库加载。对于常规链接文件，首先使用 [`RelativePath`](../../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/relativepath/) 相对于源图像路径 SourceImagePath 查找文件，如果不可用则查找 [`FullPath`](../../../aspose.psd.fileformats.psd.layers.layerresources/lifedatasource/fullpath/)，如果仍不可用则在我们的图像所在的同一目录（SourceImagePath）中查找链接文件。
...
以下代码演示了更新 Linked Smart objects 的支持。
...
// 此示例演示如何使用这些方法更新外部或嵌入的 smart object 图层：
...
void ExampleOfUpdatingSmartObjectLayer(
    ...
    FileFormat format)
{
    // 此示例演示如何更改 PSD 文件中的 smart object 图层并导出/更新其内容。
    ...
    using (PsdImage image = (PsdImage)Image.Load(filePath))
    {
        ...
        if (contentType == SmartObjectType.AvailableLinked)
        {
            ...
            // 让我们将外部 smart object 图像从 PSD smart object 图层导出到新位置
            // 因为我们将要修改它。
            smartObjectLayer.ExportContents(exportPath);
            smartObjectLayer.RelinkToFile(exportPath);
        }

        // 让我们颠倒 smart object 的内容：内部（未缓存）图像
        ...
        // 让我们检查更新后的内容是否影响渲染，以及 psd 图像是否已正确保存
        image.Save(psd2OutputPath, new PsdOptions(image));
        image.Save(png2OutputPath, new PngOptions() { ColorType = PngColorType.TruecolorWithAlpha });
    }
}

// 此示例演示如何使用 ConvertToLinked 方法将嵌入的 smart object 转换为外部链接内容。
...
void ExampleOfEmbeddedSmartObjectLayerToLinkedConversion(
    ...
    FileFormat format)
{
    // 这演示了如何将 PSD 文件中嵌入的 smart object 图层转换为外部图层。
...
// 此示例演示如何使用 EmbedLinked 方法在 PSD 文件中嵌入一个外部 smart object 图层或所有已链接图层。
...
// 此示例演示如何更改 Adobe® Photoshop® 外部 smart object 图层并导出/更新其内容
...
void ExampleOfExternalSmartObjectLayerSupport(string filePath, int contentsLength, int left, int top, int right, int bottom, FileFormat format)
{
    ...
    using (PsdImage image = (PsdImage)Image.Load(filePath))
    {
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(exportPath));
        // 让我们将已链接的 smart object 图像从 PSD smart object 图层导出
        smartObjectLayer.ExportContents(exportPath);

        // 让我们检查原始图像 isz 是否已正确保存
        image.Save(psdOutputPath, new PsdOptions(image));
        image.Save(pngOutputPath, new PngOptions() { ColorType = PngColorType.TruecolorWithAlpha });
        ...
        using (var innerImage = (RasterImage)smartObjectLayer.LoadContents(null))
        {
            ...
            // 让我们颠倒已链接的 smart object 图像
            InvertImage(innerImage);
            innerImage.Save(linkOutputPath);

            // 让我们替换 PSD 图层中已链接的 smart object 图像
