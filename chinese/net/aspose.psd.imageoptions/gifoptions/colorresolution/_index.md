---
title: GifOptions.ColorResolution
second_title: Aspose.PSD for .NET API 参考
description: GifOptions 财产. 获取或设置 GIF 颜色分辨率
type: docs
weight: 30
url: /zh/net/aspose.psd.imageoptions/gifoptions/colorresolution/
---
## GifOptions.ColorResolution property

获取或设置 GIF 颜色分辨率。

```csharp
public byte ColorResolution { get; set; }
```

### 适当的价值

颜色分辨率。

### 评论

颜色分辨率 - 原始图像中可用的每种原色 的位数，减 1。此值表示 整个调色板的大小，图形中的颜色是从中 选择的，而不是图形中实际使用的颜色数。 例如，如果此字段中的值为 3，则 原始图像的调色板有 4 位每种原色可用于创建 图像。此值应设置为指示 原始调色板的丰富性，即使源计算机上并非整个 调色板中的每种颜色都可用。

### 也可以看看

* class [GifOptions](../)
* 命名空间 [Aspose.PSD.ImageOptions](../../gifoptions/)
* 部件 [Aspose.PSD](../../../)


