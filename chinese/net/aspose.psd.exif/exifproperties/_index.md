...
| Artist | `315` | 此标签记录相机所有者、摄影师或图像创建者的姓名。详细格式没有具体说明，但为了便于互操作，建议按照下面的示例编写信息。当该字段留空时，它被视为未知。例如）“相机所有者 John Smith；摄影师 Michael Brown；图像创作者 Ken James” |
...
| YCbCrPositioning | `531` | 色度分量相对于 亮度分量的位置。该字段仅用于 JPEG 压缩数据或未压缩的 YCbCr 数据。 TIFF 默认为 1（居中）；但当 Y:Cb:Cr = 4:2:2 时，本标准建议使用 2（共址）2 来记录数据，以提高在电视系统上观看时的图像质量 。当该字段不存在时，阅读器将 假定 TIFF 默认值。在 Y:Cb:Cr = 4:2:0 的情况下，建议使用 TIFF 默认值（居中）。如果 reader 不具备同时支持这两种 YCbCrPositioning 的能力，则无论该字段中的 值如何，它都应遵循 TIFF 默认值。最好读者“ 能够同时支持居中和同位定位。 |
| ReferenceBlackWhite | `532` | 参考黑点值和参考白点 值。 TIFF 中没有给出默认值，但这里给出了以下值作为默认值。 颜色空间在颜色空间信息标签中声明 ，默认 是给出最佳图像特征的值 这些条件的互操作性 |
| Copyright | `33432` | 版权信息。在本标准中，标签用于 表示摄影师和编辑的版权。它是 个人或组织声称对图像拥有 权利的版权声明。包括日期和权利在内的互操作性版权声明 应写在this 字段中；例如，“版权所有，John Smith，19xx。保留所有权利 。”。在这个标准中，该字段记录了 摄影师和编辑的版权，每一个都记录在声明的a 单独部分中。当明确区分 摄影师和编辑版权时，这些是 按照摄影师后编辑版权的顺序写， 用NULL隔开（此时由于语句也以 一个NULL结尾，所以有两个NULL代码）。当只给出了摄影师 版权时，它以一个空代码终止。只有 给出编辑版权时，摄影师版权部分 由一个空格后跟一个终止NULL码，然后 给出编辑版权。当该字段留空时， 被视为未知。 |
...
| UserComment | `37510` | Exif 用户在 ImageDescription 之外的图像上写关键字或评论的标签，没有 ImageDescription 标签的字符代码限制。 |
...
| SpatialFrequencyResponse | `41484` | 此标签记录相机或输入设备的空间频率表和图像宽度、图像高度和对角线方向的 SFR 值，如 ISO 12233 中规定的。 |
...
| GPSLatitude | `2` | 表示纬度。纬度表示为三个 RATIONAL 值，分别给出度、分和 秒。如果纬度用度、分和秒表示，典型的格式是 dd/1,mm/1,ss/1。当使用度和分钟时，例如，分钟的小数部分最多保留两位小数，格式为 dd/1,mmmm/100,0/1. |
| GPSLongitudeRef | `3` | 表示经度是东经还是西经。 |
| GPSLongitude | `4` | 表示经度。经度表示为三个 RATIONAL 值，分别给出度、分和 秒。如果经度以度、分和秒表示，典型的格式是 ddd/1,mm/1,ss/1。当使用度数和分钟时，例如，分钟的分数最多保留两位小数点，格式为 ddd/1,mmmm/100,0/1. |
...
| GPSDestLatitude | `20` | 表示目的地的纬度。纬度表示为三个 RATIONAL 值，分别给出了 度、分和秒。如果纬度以度、分和秒表示，则典型的 格式为 dd/1,mm/1,ss/1。当使用度数和分钟时，例如，分钟的分数是 ，保留两位小数，格式为 dd/1,mmmm/100,0/1. |
...
| PixelXDimension | `40962` | 特定于压缩数据的信息。记录压缩文件时，无论是否有填充数据或重新启动标记，都应在此标记中记录有意义图像的有效宽度。 |
