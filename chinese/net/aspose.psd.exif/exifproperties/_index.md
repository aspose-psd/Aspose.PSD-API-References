...
| SamplesPerPixel | `277` | 每个像素的组件数。由于该标准适用于 RGB 和 YCbCr 图像，因此为该标签设置的值为 3。 |
...
| YCbCrPositioning | `531` | 色度分量相对于 亮度分量的位置。该字段仅用于 JPEG 压缩数据或未压缩的 YCbCr 数据。 TIFF 默认为 1（居中）；但是当 Y:Cb:Cr = 4:2:2 时， 在本标准中建议使用 2 (co-sited) 来 记录数据，以提高图像质量。在电视系统上观看 。当此字段不存在时，阅读器应 假定 TIFF 默认值。在 Y:Cb:Cr = 4:2:0 的情况下，建议使用 TIFF 默认值（居中）。如果阅读器 不具备同时支持两种 YCbCrPositioning 的能力，则无论 该字段的值如何，它都应遵循TIFF 默认值。最好让读者“ 能够同时支持居中和同位定位。 |
| ReferenceBlackWhite | `532` | 参考黑点值和参考白点 值。 TIFF 中没有给出默认值，但这里给出了以下值作为默认值。 颜色空间在颜色空间信息标签中声明为 ，默认 是提供最佳图像特征的值 这些条件的互操作性 |
| Copyright | `33432` | 版权信息。在本标准中，标签用于 表示摄影师和编辑的版权。这是 声称对图像拥有 权利的个人或组织的版权声明。包括日期和权利在内的互操作性版权声明 应写在此 字段中；例如，“版权所有，John Smith，19xx。保留所有权利 。”。在本标准中，该字段记录了 摄影师和编辑的版权，每个都记录在声明的 单独部分中。当摄影师和编辑版权明确区分 时， 按摄影师后编辑版权的顺序写， 用NULL分隔（在这种情况下）由于该语句也以 NULL 结尾，因此有两个NULL 代码）。当只给摄影师 版权时，以一个NULL码结束。当只给 编辑版权时，摄影师版权部分 由一个空格后跟一个终止NULL代码，然后 给编辑版权。当该字段留空时，它被 视为未知。 |
| ExposureTime | `33434` | 曝光时间，以秒为单位。 |
| FNumber | `33437` | F 数。 |
| ExposureProgram | `34850` | 拍摄照片时相机用来设置曝光的程序类。 |
| SpectralSensitivity | `34852` | 表示所用摄像机各通道的光谱灵敏度。 |
| PhotographicSensitivity | `34855` | 表示 ISO 12232 中指定的相机或输入设备的 ISO 感光度和 ISO 宽容度。 |
...
| CompressedBitsPerPixel | `37122` | 特定于压缩数据；表示每个像素的压缩位。 |
...
| SpatialFrequencyResponse | `41484` | 此标签记录相机或输入设备空间频率表和图像宽度、图像高度和对角线方向的 SFR 值，符合 ISO 12233 的规定。 |
...
| SubjectLocation | `41492` | 表示场景中主要主体的位置。此标签的值表示在根据旋转标签进行旋转处理之前相对于左边缘的主要主体中心的像素。 |
...
| CFAPattern | `41730` | 表示使用单芯片颜色区域传感器时图像传感器的彩色滤光片阵列（CFA）几何图案。它不适用于所有传感方法。 |
| CustomRendered | `41985` | 此标签表示对图像数据使用特殊处理，例如面向输出的渲染。当执行特殊处理时，阅读器应该禁用或最小化任何进一步的处理。 |
| ExposureMode | `41986` | 此标签表示拍摄图像时设置的曝光模式。在自动包围模式下，相机以不同的曝光设置拍摄同一场景的一系列画面。 |
| WhiteBalance | `41987` | 该标签表示拍摄图像时设置的白平衡模式。 |
| DigitalZoomRatio | `41988` | 该标签表示拍摄图像时的数码变焦倍率。如果记录值的分子为 0，则表示未使用数码变焦。 |
| FocalLengthIn35MmFilm | `41989` | 该标签表示假设 35mm 胶片相机的等效焦距，单位为 mm。值为 0 表示焦距未知。请注意，此标签不同于 FocalLength 标签。 |
...
| Sharpness | `41994` | 该标签表示拍摄图像时相机应用的锐度处理方向 |
| DeviceSettingDescription | `41995` | 此标签表示特定相机型号的拍照条件信息。标签仅用于指示阅读器中的拍照条件。 |
| SubjectDistanceRange | `41996` | 这个标签表示到主体的距离。 |
| ImageUniqueID | `42016` | 图像唯一 ID。 |
| GPSVersionID | `0` | GPSInfoIFD 版本。 |
| GPSLatitudeRef | `1` | 表示纬度是北纬还是南纬。 |
| GPSLatitude | `2` | 表示纬度。纬度表示为三个 RATIONAL 值，分别给出度、分和 秒。如果纬度以度、分和秒表示，则典型格式为 dd/1,mm/1,ss/1。当使用度数和分钟数时，例如，分钟的分数最多保留两位 小数位，格式为 dd/1,mmmm/100,0/1。 |
| GPSLongitudeRef | `3` | 表示经度是东经还是西经。 |
| GPSLongitude | `4` | 表示经度。经度表示为三个 RATIONAL 值，分别给出度、分和 秒。如果经度以度、分和秒表示，典型的格式是 ddd/1,mm/1,ss/1。当使用度数和分钟时，例如，分钟的小数部分最多保留两位 小数位，格式为 ddd/1,mmmm/100,0/1。 |
| GPSAltitudeRef | `5` | 表示用作参考高度的高度。如果参考是海平面并且海拔高度高于海平面，则给出 0。如果海拔高度低于海平面，则给出值 1，并且海拔高度在 GPSAltitude 标签中以绝对值的形式显示为 。 |
| GPSAltitude | `6` | 指示基于 GPSAltitudeRef 中的参考的高度。高度表示为一个 RATIONAL 值。 参考单位是米。 |
| GPSTimestamp | `7` | 将时间表示为 UTC（协调世界时）。 TimeStamp 表示为三个 RATIONAL 值 ，给出小时、分钟和秒。 |
...
| GPSDOP | `11` | 表示 GPS DOP（数据精度）。二维测量时写入 HDOP 值，三维测量时写入 和 PDOP。 |
| GPSSpeedRef | `12` | 表示用于表示 GPS 接收器移动速度的单位。 'K' 'M' 和 'N' 代表公里/ 小时、英里/小时和节。 |
| GPSSpeed | `13` | 表示 GPS 接收器移动的速度。 |
| GPSTrackRef | `14` | 表示 GPS 接收机运动方向的参考。 'T' 表示真实方向，'M' 是 磁方向。 |
...
| GPSDestLatitudeRef | `19` | 指示目标点的纬度是北纬还是南纬。 ASCII 值“N”表示北纬 ，“S”表示南纬。 |
| GPSDestLatitude | `20` | 表示目标点的纬度。纬度表示为三个 RATIONAL 值，分别给出 度、分和秒。如果纬度以度、分和秒表示，典型的 格式将是 dd/1,mm/1,ss/1。当使用度数和分钟数时，例如，分钟的分数为 ，保留两位小数，格式为 dd/1,mmmm/100,0/1。 |
| GPSDestLongitudeRef | `21` | 指示目标点的经度是东经还是西经。 ASCII 'E' 表示东经， 和 'W' 表示西经。 |
| GPSDestLongitude | `22` | 表示目的点的经度。经度表示为三个 RATIONAL 值，分别给出 度、分和秒。如果经度以度、分和秒表示，典型的 格式将是 ddd/1,mm/1,ss/1。当使用度数和分钟数时，例如，分钟的分数为 ，保留两位小数，格式为 ddd/1,mmmm/100,0/1。 |
| GPSDestBearingRef | `23` | 表示用于给目标点方位角的参考。 'T' 表示真实方向，'M' 是 磁方向。 |
| GPSDestBearing | `24` | 表示到目标点的方位。值的范围是从 0.00 到 359.99。 |
| GPSDestDistanceRef | `25` | 表示到目标点距离的单位。 “K”、“M”和“N”代表公里、英里 和节。 |
| GPSDestDistance | `26` | 表示到目标点的距离。 |
| GPSProcessingMethod | `27` | 一个字符串，记录用于定位的方法的名称。 第一个字节表示使用的字符代码，后面是方法的名称 。 |
