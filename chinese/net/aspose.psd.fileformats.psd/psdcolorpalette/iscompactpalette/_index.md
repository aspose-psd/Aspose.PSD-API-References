...
description: 获取一个值该值指示是否压缩它的调色板
type: docs
weight: 70
...
` true` 如果压缩调色板；否则，` false` 。

### 评论

压缩调色板意味着图像将尽可能仅包含指定的调色板条目，或者换句话说，图像将更紧凑并占用更少的空间； 否则将有 2^BitsPerPixel 条目，图像将为所有可能的调色板条目保留更多空间。 将此值设置为 true 并更改调色板条目可能会导致性能损失，因为可能会发生数据移动，因此请谨慎使用。
