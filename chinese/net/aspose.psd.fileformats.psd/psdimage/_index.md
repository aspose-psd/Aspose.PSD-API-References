...
description: 定义 PsdImage 类提供加载编辑保存 PSD 文件以及更新属性添加水印执行图形操作或将一种文件格式转换为另一种的能力 Aspose.PSD 支持作为图层导入并导出到以下格式 PngJpegJpeg2000GifBmpTiffPsdPsb 以及带有可选文本的导出到 Pdf
type: docs
weight: 3530
...
定义 PsdImage 类，提供加载、编辑、保存 PSD 文件以及更新属性、添加水印、执行图形操作或将一种文件格式转换为另一种的能力。 Aspose.PSD 支持作为图层导入并导出到以下格式： Png、Jpeg、Jpeg2000、Gif、Bmp、Tiff、Psd、Psb 以及带有可选文本的导出到 Pdf
...
| 姓名 | 描述 |
| --- | --- |
| [PsdImage](psdimage#constructor)(RasterImage) | 初始化[`PsdImage`](../psdimage)来自现有光栅图像（不是 psd 图像）的类，具有 4 通道 8 位/通道且无压缩的 RGB 颜色模式。 |
| [PsdImage](psdimage#constructor_4)(Stream) | 初始化[`PsdImage`](../psdimage)来自光栅图像的指定路径的类（不是流中的 psd 图像）。用于使用默认参数初始化 psd 图像 - 颜色模式 - rgb，4 通道，每通道 8 位，压缩 - Raw. |
| [PsdImage](psdimage#constructor_6)(string) | 初始化[`PsdImage`](../psdimage)来自光栅图像的指定路径的类（不是路径中的 psd 图像）。用于使用默认参数初始化 psd 图像 - 颜色模式 - rgb，4 通道，每通道 8 位，压缩 - Raw. |
| [PsdImage](psdimage#constructor_2)(int, int) | 初始化[`PsdImage`](../psdimage)具有指定宽度和高度的类。用于初始化空psd图像。 |
| [PsdImage](psdimage#constructor_1)(RasterImage, ColorModes, short, short, int, CompressionMethod) | 初始化[`PsdImage`](../psdimage)具有构造函数参数的现有光栅图像（不是 psd 图像）的类。 |
| [PsdImage](psdimage#constructor_5)(Stream, ColorModes, short, short, int, CompressionMethod) | 初始化[`PsdImage`](../psdimage)具有构造函数参数的光栅图像（不是流中的 psd 图像）的指定路径中的类。 |
| [PsdImage](psdimage#constructor_7)(string, ColorModes, short, short, int, CompressionMethod) | 初始化[`PsdImage`](../psdimage)带有构造函数参数的光栅图像（不是路径中的 psd 图像）的指定路径中的类。 |
| [PsdImage](psdimage#constructor_3)(int, int, IColorPalette, ColorModes, short, short, int, CompressionMethod) | 初始化[`PsdImage`](../psdimage)具有指定宽度、高度、调色板、颜色模式、通道数和通道位长度以及指定压缩模式参数的类。用于初始化空psd图像。 |
...
| override [BitsPerPixel](../../aspose.psd.fileformats.psd/psdimage/bitsperpixel) { get; } | 获取每像素计数的图像位数。 |
...
| override [HasAlpha](../../aspose.psd.fileformats.psd/psdimage/hasalpha) { get; } | 获取或设置此对象的垂直分辨率，以每英寸像素为单位[`RasterImage`](../../aspose.psd/rasterimage) . |
| virtual [HasBackgroundColor](../../aspose.psd/image/hasbackgroundcolor) { get; set; } | 获取或设置图像是否有背景色的值。 |
| [HasTransparencyData](../../aspose.psd.fileformats.psd/psdimage/hastransparencydata) { get; set; } | 获取或设置一个值，该值指示第一个alpha通道是否包含指定图层数据时合并结果的透明度数据。 |
...
| override [HorizontalResolution](../../aspose.psd.fileformats.psd/psdimage/horizontalresolution) { get; set; } | 获取或设置此图像的水平分辨率，以每英寸像素为单位[`PsdImage`](../psdimage) . |
| virtual [ImageOpacity](../../aspose.psd/rasterimage/imageopacity) { get; } | 获取此图像的不透明度。 |
| [ImageResources](../../aspose.psd.fileformats.psd/psdimage/imageresources) { get; set; } | 获取或设置PSD图片资源。 |
...
| override [VerticalResolution](../../aspose.psd.fileformats.psd/psdimage/verticalresolution) { get; set; } | 获取或设置此对象的垂直分辨率，以每英寸像素为单位[`PsdImage`](../psdimage) . |
...
| [AddInvertAdjustmentLayer](../../aspose.psd.fileformats.psd/psdimage/addinvertadjustmentlayer)() | 添加一个反转调整层。 |
...
| [AddPhotoFilterLayer](../../aspose.psd.fileformats.psd/psdimage/addphotofilterlayer)(Color) | 添加 PhotoFilter 层。 |
...
| override [AdjustContrast](../../aspose.psd.fileformats.psd/psdimage/adjustcontrast)(float) | 图像对比 |
| override [AdjustGamma](../../aspose.psd.fileformats.psd/psdimage/adjustgamma#adjustgamma)(float) | 图像的 Gamma 校正。 |
| override [AdjustGamma](../../aspose.psd.fileformats.psd/psdimage/adjustgamma#adjustgamma_1)(float, float, float) | 图像的 Gamma 校正。 |
...
| override [BinarizeFixed](../../aspose.psd.fileformats.psd/psdimage/binarizefixed)(byte) | 具有预定义阈值的图像二值化 |
...
| override [ReplaceNonTransparentColors](../../aspose.psd.fileformats.psd/psdimage/replacenontransparentcolors#replacenontransparentcolors_1)(int) | 用新颜色替换所有不透明的颜色，并保留原始的 alpha 值以保存平滑的边缘。 注意：如果在不透明的图像上使用它，所有颜色都将替换为一个颜色。 |
...
| override [Save](../../aspose.psd/rasterimage/save)(Stream, ImageOptionsBase, Rectangle) | 根据保存选项将图像的数据以指定的文件格式保存到指定的流中。 |
...
| 姓名 | 描述 |
| --- | --- |
| const [DefaultVersion](../../aspose.psd.fileformats.psd/psdimage/defaultversion) | 默认的PSD版本。 |
