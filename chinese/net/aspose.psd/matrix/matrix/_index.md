---
title: Matrix
second_title: Aspose.PSD for .NET API 参考
description: 将 Matrix 类的新实例初始化为单位矩阵
type: docs
weight: 10
url: /zh/net/aspose.psd/matrix/matrix/
---
## Matrix() {#constructor}

将 Matrix 类的新实例初始化为单位矩阵。

```csharp
public Matrix()
```

### 也可以看看

* class [Matrix](../../matrix)
* 命名空间 [Aspose.PSD](../../matrix)
* 部件 [Aspose.PSD](../../../)

---

## Matrix(float, float, float, float, float, float) {#constructor_4}

初始化[`Matrix`](../../matrix)类的新实例。

```csharp
public Matrix(float m11, float m12, float m21, float m22, float m31, float m32)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| m11 | Single | m00 M11 比例 X |
| m12 | Single | m10 M12 剪切 Y |
| m21 | Single | m01 M21 剪切 X |
| m22 | Single | m11 M22 比例 Y |
| m31 | Single | m02 M31 平移 X |
| m32 | Single | m12 M32 平移 Y |

### 也可以看看

* class [Matrix](../../matrix)
* 命名空间 [Aspose.PSD](../../matrix)
* 部件 [Aspose.PSD](../../../)

---

## Matrix(RectangleF, PointF[]) {#constructor_3}

将[`Matrix`](../../matrix)类的新实例初始化为由指定矩形和点数组定义的几何变换。

```csharp
public Matrix(RectangleF rect, PointF[] plgpts)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| rect | RectangleF | [`RectangleF`](../../rectanglef)结构，表示要转换的矩形。 |
| plgpts | PointF[] | 三个[`PointF`](../../pointf)结构的数组，表示左上角、右上角、矩形的左下角将被变换。平行四边形的右下角由前三个角隐含。 |

### 也可以看看

* struct [RectangleF](../../rectanglef)
* struct [PointF](../../pointf)
* class [Matrix](../../matrix)
* 命名空间 [Aspose.PSD](../../matrix)
* 部件 [Aspose.PSD](../../../)

---

## Matrix(Rectangle, Point[]) {#constructor_2}

将[`Matrix`](../../matrix)类的新实例初始化为由指定矩形和点数组定义的几何变换。

```csharp
public Matrix(Rectangle rect, Point[] plgpts)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| rect | Rectangle | [`Rectangle`](../../rectangle)结构，表示要转换的矩形。 |
| plgpts | Point[] | 由三个[`Point`](../../point)结构组成的数组，表示平行四边形的左上、右上、矩形的左下角将被变换。平行四边形的右下角由前三个角隐含。 |

### 也可以看看

* struct [Rectangle](../../rectangle)
* struct [Point](../../point)
* class [Matrix](../../matrix)
* 命名空间 [Aspose.PSD](../../matrix)
* 部件 [Aspose.PSD](../../../)

---

## Matrix(Matrix) {#constructor_1}

复制[`Matrix`](../../matrix)类。

```csharp
public Matrix(Matrix origin)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| origin | Matrix | 应对的基本矩阵 |

### 也可以看看

* class [Matrix](../../matrix)
* 命名空间 [Aspose.PSD](../../matrix)
* 部件 [Aspose.PSD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
