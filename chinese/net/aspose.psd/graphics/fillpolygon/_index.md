---
title: FillPolygon
second_title: Aspose.PSD for .NET API 参考
description: 填充由PointFaspose.psd/pointf结构和Alternate
type: docs
weight: 380
url: /zh/net/aspose.psd/graphics/fillpolygon/
---
## FillPolygon(Brush, PointF[]) {#fillpolygon}

填充由[`PointF`](../../pointf)结构和Alternate。

```csharp
public void FillPolygon(Brush brush, PointF[] points)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| brush | Brush | [`Brush`](../../brush)确定填充的特性。 |
| points | PointF[] | [`PointF`](../../pointf)结构的数组，表示要填充的多边形的顶点。 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *brush*为空。 - 或 - *points*为空。 |

### 也可以看看

* class [Brush](../../brush)
* struct [PointF](../../pointf)
* class [Graphics](../../graphics)
* 命名空间 [Aspose.PSD](../../graphics)
* 部件 [Aspose.PSD](../../../)

---

## FillPolygon(Brush, PointF[], FillMode) {#fillpolygon_1}

使用指定的填充模式填充由[`PointF`](../../pointf)结构指定的点数组定义的多边形内部。

```csharp
public void FillPolygon(Brush brush, PointF[] points, FillMode fillMode)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| brush | Brush | [`Brush`](../../brush)确定填充的特性。 |
| points | PointF[] | [`PointF`](../../pointf)结构的数组，表示要填充的多边形的顶点。 |
| fillMode | FillMode | 确定填充样式的[`FillMode`](../../fillmode)枚举成员。 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *brush*为空。 - 或 - *points*为空。 |

### 也可以看看

* class [Brush](../../brush)
* struct [PointF](../../pointf)
* enum [FillMode](../../fillmode)
* class [Graphics](../../graphics)
* 命名空间 [Aspose.PSD](../../graphics)
* 部件 [Aspose.PSD](../../../)

---

## FillPolygon(Brush, Point[]) {#fillpolygon_2}

填充由[`Point`](../../point)结构和Alternate。

```csharp
public void FillPolygon(Brush brush, Point[] points)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| brush | Brush | [`Brush`](../../brush)确定填充的特性。 |
| points | Point[] | [`Point`](../../point)结构的数组，表示要填充的多边形的顶点。 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *brush*为空。 - 或 - *points*为空。 |

### 也可以看看

* class [Brush](../../brush)
* struct [Point](../../point)
* class [Graphics](../../graphics)
* 命名空间 [Aspose.PSD](../../graphics)
* 部件 [Aspose.PSD](../../../)

---

## FillPolygon(Brush, Point[], FillMode) {#fillpolygon_3}

使用指定的填充模式填充由[`Point`](../../point)结构指定的点数组定义的多边形内部。

```csharp
public void FillPolygon(Brush brush, Point[] points, FillMode fillMode)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| brush | Brush | [`Brush`](../../brush)确定填充的特性。 |
| points | Point[] | [`Point`](../../point)结构的数组，表示要填充的多边形的顶点。 |
| fillMode | FillMode | 确定填充样式的[`FillMode`](../../fillmode)枚举成员。 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *brush*为空。 - 或 - *points*为空。 |

### 也可以看看

* class [Brush](../../brush)
* struct [Point](../../point)
* enum [FillMode](../../fillmode)
* class [Graphics](../../graphics)
* 命名空间 [Aspose.PSD](../../graphics)
* 部件 [Aspose.PSD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
