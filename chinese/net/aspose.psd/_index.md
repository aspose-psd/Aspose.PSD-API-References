...
description: 命名空间是嵌套命名空间的核心也是Aspose.PSD处理中使用的最基本的对象
type: docs
weight: 10
...
| [Blend](./blend/) | 定义混合模式。此类不能被继承。 |
| [Brush](./brush/) | 基础画笔类。 |
| [BuildVersionInfo](./buildversioninfo/) | 包含当前构建版本信息。 |
| [Cache](./cache/) | 包含缓存设置。 |
| [CmykColorHelper](./cmykcolorhelper/) | 辅助方法使用 CMYK 颜色作为带符号的 32 位整数值。 提供与[`CmykColor`](../aspose.psd/cmykcolor/)struct. 它更轻量级，因为 CMYK 颜色以 Int32 形式呈现，而不是具有内部字段的结构。 请尽可能使用此类的静态方法，而不是 deprecated [`CmykColor`](../aspose.psd/cmykcolor/)结构. |
| [ColorBlend](./colorblend/) | 定义用于在多色渐变中插值颜色混合的颜色和位置数组。此类不能被继承。 |
| [ColorMap](./colormap/) | 定义用于转换颜色的映射。的几种方法[`ImageAttributes`](../aspose.psd/imageattributes/)类通过使用颜色重新映射表来调整图像颜色，该表是一个数组[`ColorMap`](../aspose.psd/colormap/)结构。不可继承. |
| [ColorMatrix](./colormatrix/) | 定义一个 5 x 5 矩阵，其中包含 RGBA 空间的坐标。的几种方法[`ImageAttributes`](../aspose.psd/imageattributes/)类使用颜色矩阵调整图像颜色。此类不能被继承。 |
| [ColorPalette](./colorpalette/) | 定义构成调色板的颜色数组。颜色是 32 位 ARGB 颜色。不可继承. |
| [ColorPaletteHelper](./colorpalettehelper/) | 调色板操作的辅助类。 |
| [ColorTranslator](./colortranslator/) | 将颜色与 GDI+ 颜色结构相互转换。此类不能被继承。 |
| [CompositeException](./compositeexception/) | 复合异常 |
| [CustomLineCap](./customlinecap/) | 封装一个自定义的用户定义行 cap. |
...
| [Font](./font/) | 定义文本的特定格式，包括字体、大小和样式属性。此类不能被继承。 |
| [FontSettings](./fontsettings/) | 通用 PSD 矢量格式渲染器字体设置。 |
| [Graphics](./graphics/) | 表示根据当前程序集使用的图形引擎的图形。 |
| [GraphicsPath](./graphicspath/) | 代表一系列相连的直线和曲线。此类不能被继承。 |
| [Image](./image/) | 图像是所有类型图像的基类。 |
| [ImageAttributes](./imageattributes/) | 一个[`ImageAttributes`](../aspose.psd/imageattributes/)对象包含有关位图和图元文件颜色在渲染过程中如何操作的信息。一个[`ImageAttributes`](../aspose.psd/imageattributes/)对象维护多个颜色调整设置，包括颜色调整矩阵、灰度调整矩阵、伽玛校正值、颜色映射表和颜色阈值。在渲染过程中，可以校正、变暗、变亮和移除颜色。要应用此类操作，请初始化一个[`ImageAttributes`](../aspose.psd/imageattributes/)对象并传递它的路径[`ImageAttributes`](../aspose.psd/imageattributes/)对象（连同路径[`Image`](../aspose.psd/image/) 到 DrawImage 方法。 |
...
| [ImageResizeSettings](./imageresizesettings/) | 图像调整大小设置 class |
...
| [Size](./size/) | 代表尺寸。 |
| [SizeF](./sizef/) | 存储一对有序的浮点数，通常是矩形的宽度和高度。 |
## 接口
...
| [DashCap](./dashcap/) | 指定要在虚线中每个破折号两端使用的图形形状类型。 |
