...
description: Aspose.PSD.FileFormats.Psd.Layers.LayerResources.LiFdDataSource 班级. 在 PSD 文件中定义 liFD 数据源类其中包含有关嵌入文件的信息 这是 PSD 文件格式操作 API 的一部分有助于修改 Adobe Photoshop 文件
type: docs
weight: 2670
...
在 PSD 文件中定义 liFD 数据源类，其中包含有关嵌入文件的信息。 这是 PSD 文件格式操作 API 的一部分，有助于修改 Adobe® Photoshop® 文件
...
| 姓名 | 描述 |
| --- | --- |
| [LiFdDataSource](lifddatasource/#constructor)() | 初始化一个新的实例`LiFdDataSource`类. |
| [LiFdDataSource](lifddatasource/#constructor_1)(int, Guid, string, string, string) | 初始化一个新的实例`LiFdDataSource`类. |
