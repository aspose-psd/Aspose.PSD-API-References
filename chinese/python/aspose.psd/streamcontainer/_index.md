...
type: docs
weight: 4230
url: /zh/python-net/aspose.psd/streamcontainer/
---
...
| **Name** | **Description** |
| :- | :- |
| [StreamContainer(stream)](#StreamContainer_stream_1) | 初始化一个新的 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/) 类实例。 |
| [StreamContainer(stream, dispose_stream)](#StreamContainer_stream_dispose_stream_2) | 初始化一个新的 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/) 类实例。 |
## **Properties**
...
| [read_byte()](#read_byte__3) | 从流中读取一个字节，并将流中的位置前移一个字节；如果已到达流末尾，则返回 -1。 |
| [save(destination_stream)](#save_destination_stream_4) | 将流的数据保存（复制）到指定的流。使用默认缓冲区大小 [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/) 和流的 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 值。 |
| [save(destination_stream, buffer_size)](#save_destination_stream_buffer_size_5) | 将流的全部数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 值。 |
| [save(destination_stream, buffer_size, length)](#save_destination_stream_buffer_size_length_6) | 将流的数据保存（复制）到指定的流。 |
| [save(file_path)](#save_file_path_7) | 将流的数据保存（复制）到指定的流。使用默认缓冲区大小 [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/) 和流的 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 值。 |
| [save(file_path, buffer_size)](#save_file_path_buffer_size_8) | 将流的数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/) 值。 |
| [save(file_path, buffer_size, length)](#save_file_path_buffer_size_length_9) | 将流的数据保存（复制）到指定的流。 |
| [seek(offset, origin)](#seek_offset_origin_10) | 设置当前流中的位置。 |
| seek_begin() | 将流位置设置为流的开头。此值表示相对于在 StreamContainer 构造函数中传入的起始流位置的偏移量。 |
| [to_bytes()](#to_bytes__11) | 将流数据转换为 int 数组。 |
| [to_bytes(position, bytes_count)](#to_bytes_position_bytes_count_12) | 将流数据转换为 int 数组。 |
| [write(buffer, offset, count)](#write_buffer_offset_count_13) | 将一系列字节写入当前流，并根据写入的字节数前移该流中的当前位置。 |
| [write(bytes)](#write_bytes_14) | 将所有指定的字节写入流。 |
| [write_byte(value)](#write_byte_value_15) | 在流的当前位置写入一个字节，并将流中的位置前移一个字节。 |
| [write_to(stream_container)](#write_to_stream_container_16) | 将包含的数据复制到另一个 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/)。 |
| [write_to(stream_container, length)](#write_to_stream_container_length_17) | 将包含的数据复制到另一个 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/)。 |
...
初始化一个新的 [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/) 类实例。
