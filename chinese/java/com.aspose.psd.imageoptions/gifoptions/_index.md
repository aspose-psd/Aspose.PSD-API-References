...
type: docs
weight: 12
url: /zh/java/com.aspose.psd.imageoptions/gifoptions/
---
...
| [GifOptions(GifOptions gifOptions)](#GifOptions-com.aspose.psd.imageoptions.GifOptions-) | 初始化 GifOptions 类的新实例。 |
## Methods
...
| [getDoPaletteCorrection()](#getDoPaletteCorrection--) | 获取或设置一个指示是否应用调色板校正的值。 |
...
| [setDoPaletteCorrection(boolean value)](#setDoPaletteCorrection-boolean-) | 获取或设置一个指示是否应用调色板校正的值。 |
...
| [setInterlaced(boolean value)](#setInterlaced-boolean-) | 如果图像应交错则为 True。 |
...
| [setPixelAspectRatio(byte value)](#setPixelAspectRatio-byte-) | 获取或设置 GIF 像素纵横比。 |
...
| Parameter | Type | 描述 |
| --- | --- | --- |
| gifOptions | [GifOptions](../../com.aspose.psd.imageoptions/gifoptions) | GIF 选项。 |
...
颜色分辨率 - 原始图像每个原色可用的位数减 1。此值表示用于从中选择图形颜色的整个调色板的大小，而不是图形实际使用的颜色数量。例如，如果此字段的值为 3，则原始图像的调色板每个原色有 4 位可用于创建图像。即使并非所有调色板中的颜色都在源机器上可用，也应设置此值以指示原始调色板的丰富程度。
...
**Returns:**
boolean -  true  如果已应用调色板校正；否则为  false 。

调色板校正意味着每当图像导出为 GIF 时，源图像的颜色将被分析以构建最匹配的调色板（如果图像调色板不存在或未在选项中指定）。分析过程需要一些时间，但输出图像将拥有最匹配的颜色调色板，视觉效果更佳。
...
获取或设置允许的最大像素差异。如果大于零，将使用有损压缩。针对最佳有损压缩的推荐值为 80。30 表示非常轻的压缩，200 表示较重的压缩。仅在引入的损失很小的情况下效果最佳，并且由于压缩算法的限制，极高的损失水平不会带来太多收益。允许的取值范围为 [0, 1000]。
...
像素纵横比 - 用于计算原始图像中像素纵横比近似值的因子。如果字段值不为 0，则根据公式计算该近似纵横比：Aspect Ratio = (Pixel Aspect Ratio + 15) / 64。像素纵横比定义为像素宽度除以高度的商。此字段的取值范围允许以 1/64 为增量指定最宽像素 4:1 到最高像素 1:4。取值：0 - 未提供纵横比信息。1..255 - 用于计算的值。
...
**Returns:**
boolean -  true  如果 GIF 有尾部；否则为  false 。
### hashCode() {#hashCode--}
```
...
**Returns:**
boolean -  true  如果调色板条目已排序；否则为  false 。
### notify() {#notify--}
```
...
| Parameter | Type | 描述 |
| --- | --- | --- |
|  | 值 | boolean | true  如果已应用调色板校正；否则为  false 。 |
...
| Parameter | Type | 描述 |
| --- | --- | --- |
| 值 | boolean | true  如果调色板条目已排序；否则为  false 。 |
...
| Parameter | Type | 描述 |
| --- | --- | --- |
| 值 | boolean | true  如果 GIF 有尾部；否则为  false 。 |
