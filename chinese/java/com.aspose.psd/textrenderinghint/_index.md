...
type: docs
weight: 109
url: /zh/java/com.aspose.psd/textrenderinghint/
---
...
每个字符使用其无提示的抗锯齿字形位图绘制。由于抗锯齿，质量更好。由于关闭提示，可能会注意到笔画宽度差异。
...
每个字符使用其带提示的抗锯齿字形位图绘制。由于抗锯齿，质量大幅提升，但性能开销更高。
...
每个字符都使用其字形位图绘制。Hinting 用于改善字符在笔画和曲线上的外观。
...
每个字符都使用其字形位图绘制，并使用系统默认的渲染提示。文本将按照用户为系统选择的任何 font-smoothing 设置进行绘制。
