...
type: docs
weight: 37
url: /zh/java/com.aspose.psd/datarecoverymode/
---
...
| 字段 | 描述 |
| --- | --- |
| [ConsistentRecover](#ConsistentRecover) | 一致恢复模式会尝试恢复所有数据，只要损坏未破坏文件格式，并且允许正确的后续处理。 |
| [EnumSeparatorCharArray](#EnumSeparatorCharArray) |  |
| [MaximalRecover](#MaximalRecover) | 最大恢复模式会恢复所有数据，即使文件格式结构已损坏，后续处理可能会产生不可预料的效果。 |
| [None](#None) | 不进行数据恢复。 |
## Methods
...
一致恢复模式会尝试恢复所有数据，只要损坏未破坏文件格式，并且允许正确的后续处理。
...
最大恢复模式会恢复所有数据，即使文件格式结构已损坏，后续处理可能会产生不可预料的效果。
...
不进行数据恢复。每当文件格式存在某些损坏数据时，将抛出相应的异常。
