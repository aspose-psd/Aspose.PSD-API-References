---
title: "Classe IPathShape"
type: docs
weight: 380
url: /it/python-net/aspose.psd.fileformats.psd.layers.layerresources/ipathshape/
---

**Summary:** The Shape from the knots of the Bezier curve.

**Module:** [aspose.psd.fileformats.psd.layers.layerresources](/psd/python-net/aspose.psd.fileformats.psd.layers.layerresources/)

**Full Name:** aspose.psd.fileformats.psd.layers.layerresources.IPathShape

**Aspose.PSD Version:** 24.12.0

## **Properties**
| **Name** | **Type** | **Access** | **Description** |
| :- | :- | :- | :- |
| is_closed | bool | r/w | Ottiene o imposta la proprietà che determina se la Shape è chiusa. |
| path_operations | [PathOperations](/psd/python-net/aspose.psd.fileformats.core.vectorpaths/pathoperations/) | r/w | Le operazioni per la combinazione delle forme di percorso (operazioni booleane). |
## **Methods**
| **Name** | **Description** |
| :- | :- |
| [get_items()](#get_items__1) | Ottiene l'array di nodi Bezier. |
| [set_items(bezier_points)](#set_items_bezier_points_2) | Assegna un array di nodi Bexier. |


### Method: get_items() {#get_items__1}


```
 get_items() 
```

Ottiene l'array di nodi Bezier.

**Returns**

| Tipo | Descrizione |
| :- | :- |
| [BezierKnotRecord[]](/psd/python-net/aspose.psd.fileformats.core.vectorpaths/bezierknotrecord/) | Array di BezierKnotRecord. |


### Method: set_items(bezier_points) {#set_items_bezier_points_2}


```
 set_items(bezier_points) 
```

Assegna un array di nodi Bexier.

**Parameters:**

| Parametro | Tipo | Descrizione |
| :- | :- | :- |
| bezier_points | [BezierKnotRecord[]](/psd/python-net/aspose.psd.fileformats.core.vectorpaths/bezierknotrecord/) | Array di nodi Bézier |

