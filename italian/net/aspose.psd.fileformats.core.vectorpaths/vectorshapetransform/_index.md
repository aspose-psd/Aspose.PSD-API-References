...
description: Aspose.PSD.FileFormats.Core.VectorPaths.VectorShapeTransform classe. Definisce la matrice di trasformazione della forma vettoriale class
type: docs
weight: 1460
...
| Nome | Descrizione |
| --- | --- |
| [VectorShapeTransform](vectorshapetransform/)() | Inizializza una nuova istanza di`VectorShapeTransform` classe. |
...
using (PsdImage image = (PsdImage)Image.Load(sourceFileName))
{
    ...
    // Salva questa immagine PSD con le proprietà modificate.
...
using (PsdImage image = (PsdImage)Image.Load(outputPath))
{
    ...
    // Asserisce che le proprietà vengono salvate e caricate correttamente 
    var setting = resource.ShapeOriginSettings[0];
    AssertAreEqual(true, setting.IsOriginIndexPresent);
    AssertAreEqual(false, setting.IsShapeInvalidatedPresent);
    AssertAreEqual(true, setting.IsOriginResolutionPresent);
