---
title: PrintScaleResource
second_title: Riferimento all'API di Aspose.PSD per .NET
description: Risorsa scala di stampa
type: docs
weight: 3780
url: /it/net/aspose.psd.fileformats.psd.resources/printscaleresource/
---
## PrintScaleResource class

Risorsa scala di stampa

```csharp
public sealed class PrintScaleResource : ResourceBlock
```

## Costruttori

| Nome | Descrizione |
| --- | --- |
| [PrintScaleResource](printscaleresource)() | Default_Costruttore |

## Proprietà

| Nome | Descrizione |
| --- | --- |
| override [DataSize](../../aspose.psd.fileformats.psd.resources/printscaleresource/datasize) { get; } | Ottiene la dimensione dei dati della risorsa in byte. |
| [ID](../../aspose.psd.fileformats.psd/resourceblock/id) { get; set; } | Ottiene o imposta l'identificatore univoco per la risorsa. |
| override [MinimalVersion](../../aspose.psd.fileformats.psd.resources/printscaleresource/minimalversion) { get; } | Ottiene la versione PSD minima richiesta. |
| [Name](../../aspose.psd.fileformats.psd/resourceblock/name) { get; set; } | Ottiene o imposta il nome della risorsa. Stringa Pascal, imbottita per rendere uniforme la dimensione (un nome null è costituito da due byte di 0). |
| [Scale](../../aspose.psd.fileformats.psd.resources/printscaleresource/scale) { get; set; } | Ottiene o imposta la scala. |
| [Signature](../../aspose.psd.fileformats.psd/resourceblock/signature) { get; } | Ottiene la firma della risorsa. Dovrebbe essere sempre '8BIM'. |
| [Size](../../aspose.psd.fileformats.psd/resourceblock/size) { get; } | Ottiene la dimensione del blocco di risorse in byte inclusi i suoi dati. |
| [Style](../../aspose.psd.fileformats.psd.resources/printscaleresource/style) { get; set; } | Ottiene o imposta lo stile. |
| [XLocation](../../aspose.psd.fileformats.psd.resources/printscaleresource/xlocation) { get; set; } | Ottiene o imposta la posizione x. |
| [YLocation](../../aspose.psd.fileformats.psd.resources/printscaleresource/ylocation) { get; set; } | Ottiene o imposta la posizione y. |

## Metodi

| Nome | Descrizione |
| --- | --- |
| [Save](../../aspose.psd.fileformats.psd/resourceblock/save)(StreamContainer) | Salva il blocco di risorse nel flusso specificato. |
| virtual [ValidateValues](../../aspose.psd.fileformats.psd/resourceblock/validatevalues)() | Convalida i valori delle risorse. |

### Guarda anche

* class [ResourceBlock](../../aspose.psd.fileformats.psd/resourceblock)
* spazio dei nomi [Aspose.PSD.FileFormats.Psd.Resources](../../aspose.psd.fileformats.psd.resources)
* assemblea [Aspose.PSD](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
