---
title: UnknownStructure
second_title: Riferimento all'API di Aspose.PSD per .NET
description: La struttura sconosciuta.
type: docs
weight: 3240
url: /it/net/aspose.psd.fileformats.psd.layers.layerresources.typetoolinfostructures/unknownstructure/
---
## UnknownStructure class

La struttura sconosciuta.

```csharp
public sealed class UnknownStructure : OSTypeStructure
```

## Costruttori

| Nome | Descrizione |
| --- | --- |
| [UnknownStructure](unknownstructure)(ClassID, int) | Inizializza una nuova istanza di[`UnknownStructure`](../unknownstructure) classe. |

## Proprietà

| Nome | Descrizione |
| --- | --- |
| [Data](../../aspose.psd.fileformats.psd.layers.layerresources.typetoolinfostructures/unknownstructure/data) { get; set; } | Ottiene o imposta i dati. |
| override [Key](../../aspose.psd.fileformats.psd.layers.layerresources.typetoolinfostructures/unknownstructure/key) { get; } | Ottiene la chiave della struttura. |
| [KeyName](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure/keyname) { get; set; } | Ottiene o imposta il nome della chiave. |
| override [Length](../../aspose.psd.fileformats.psd.layers.layerresources.typetoolinfostructures/unknownstructure/length) { get; } | Ottiene il[`OSTypeStructure`](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure) lunghezza in byte. |

## Metodi

| Nome | Descrizione |
| --- | --- |
| virtual [GetHeaderLength](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure/getheaderlength)() | Ottiene la lunghezza dell'intestazione. |
| [Save](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure/save)(StreamContainer) | Salva la struttura nel contenitore del flusso specificato. |
| [SaveWithoutKeyName](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure/savewithoutkeyname)(StreamContainer) | Salva la struttura nel contenitore del flusso specificato. |

### Guarda anche

* class [OSTypeStructure](../../aspose.psd.fileformats.psd.layers.layerresources/ostypestructure)
* spazio dei nomi [Aspose.PSD.FileFormats.Psd.Layers.LayerResources.TypeToolInfoStructures](../../aspose.psd.fileformats.psd.layers.layerresources.typetoolinfostructures)
* assemblea [Aspose.PSD](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PSD.dll -->
