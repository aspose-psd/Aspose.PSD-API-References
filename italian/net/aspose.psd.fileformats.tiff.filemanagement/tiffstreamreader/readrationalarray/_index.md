---
title: TiffStreamReader.ReadRationalArray
second_title: Aspose.PSD per riferimento API .NET
description: TiffStreamReader metodo. Legge un array di valori razionali dallo stream.
type: docs
weight: 100
url: /it/net/aspose.psd.fileformats.tiff.filemanagement/tiffstreamreader/readrationalarray/
---
## TiffStreamReader.ReadRationalArray method

Legge un array di valori razionali dallo stream.

```csharp
public TiffRational[] ReadRationalArray(long position, long count)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| position | Int64 | La posizione da cui leggere. |
| count | Int64 | Gli elementi contano. |

### Valore di ritorno

L'array di valori razionali.

### Eccezioni

| eccezione | condizione |
| --- | --- |
| ArgumentOutOfRangeException | contare;Previsto un numero positivo. |

### Guarda anche

* class [TiffRational](../../../aspose.psd.fileformats.tiff/tiffrational/)
* class [TiffStreamReader](../)
* spazio dei nomi [Aspose.PSD.FileFormats.Tiff.FileManagement](../../tiffstreamreader/)
* assemblea [Aspose.PSD](../../../)


