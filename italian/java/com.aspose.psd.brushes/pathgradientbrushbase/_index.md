...
description: "Rappresenta un pennello con funzionalità di gradiente di percorso di base."
type: docs
weight: 15
url: /it/java/com.aspose.psd.brushes/pathgradientbrushbase/
---
...
Rappresenta un  Brush  con funzionalità di gradiente a percorso di base.

Nota che quando si crea la classe  PathGradientBrushBase  dovrebbe essere inizializzata con almeno 2 punti. Il percorso interno creato sarà sempre una figura chiusa, l'ultimo punto collega il primo punto. Quella forma è riempita con questo  PathGradientBrushBase . L'implementazione GDI+ genera un  OutOfMemoryError  quando si passano array vuoti o insiemi di punti con le stesse coordinate. Il  PathGradientBrushBase  genera un'eccezione quando l'array di punti contiene meno di 2 punti, viene lanciato un  ArgumentException  anziché un  OutOfMemoryError  quando l'array di punti è inaccettabile. Il punto centrale è calcolato come centro di massa dei punti forniti per impostazione predefinita. L'utente può modificare questo punto in seguito. La scala di messa a fuoco è un punto vuoto (0.0, 0.0) per impostazione predefinita.
## Metodi
