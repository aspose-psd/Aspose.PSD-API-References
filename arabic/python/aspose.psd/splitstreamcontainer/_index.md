...
type: docs
weight: 4220
url: /ar/python-net/aspose.psd/splitstreamcontainer/
---
...
| **Name** | **الوصف** |
| :- | :- |
| [SplitStreamContainer(stream)](#SplitStreamContainer_stream_1) | يقوم بتهيئة نسخة جديدة من الفئة [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/). |
| [SplitStreamContainer(stream, dispose_stream)](#SplitStreamContainer_stream_dispose_stream_2) | يقوم بتهيئة نسخة جديدة من الفئة [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/). |
| [SplitStreamContainer(stream, dispose_stream)](#SplitStreamContainer_stream_dispose_stream_3) | يقوم بتهيئة نسخة جديدة من الفئة [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/). |
## **Properties**
...
| [insert(position, stream, dispose_stream)](#insert_position_stream_dispose_stream_1) | يدرج حاوية التدفق في الموضع المحدد. |
| [read(buffer, offset, count)](#read_buffer_offset_count_2) | يقرأ تسلسلًا من البايتات من التدفق الحالي ويقدم الموضع داخل التدفق بعدد البايتات المقروءة. |
| [read(bytes)](#read_bytes_3) | يقرأ بايتات لملء المخزن المؤقت للبايتات المحدد. |
| [read_byte()](#read_byte__4) | يقرأ بايتًا من التدفق ويقدم الموضع داخل التدفق بايتًا واحدًا، أو يُعيد -1 إذا كان عند نهاية التدفق. |
| [save(destination_stream)](#save_destination_stream_5) | يحفظ (ينسخ) بيانات التدفق إلى التدفق المحدد. يستخدم حجم المخزن المؤقت الافتراضي [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/) وقيمة [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/). |
| [save(destination_stream, buffer_size)](#save_destination_stream_buffer_size_6) | يحفظ (ينسخ) جميع بيانات التدفق إلى التدفق المحدد. يستخدم قيمة [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/). |
| [save(destination_stream, buffer_size, length)](#save_destination_stream_buffer_size_length_7) | يحفظ (ينسخ) بيانات التدفق إلى التدفق المحدد. |
| [save(file_path)](#save_file_path_8) | يحفظ (ينسخ) بيانات التدفق إلى التدفق المحدد. يستخدم حجم المخزن المؤقت الافتراضي [StreamContainer.READ_WRITE_BYTES_COUNT](/psd/python-net/aspose.psd/streamcontainer/) وقيمة [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/). |
| [save(file_path, buffer_size)](#save_file_path_buffer_size_9) | يحفظ (ينسخ) بيانات الدفق إلى الدفق المحدد. يستخدم قيمة الدفق [StreamContainer.length](/psd/python-net/aspose.psd/streamcontainer/). |
| [save(file_path, buffer_size, length)](#save_file_path_buffer_size_length_10) | يحفظ (ينسخ) بيانات التدفق إلى التدفق المحدد. |
| [seek(offset, origin)](#seek_offset_origin_11) | يضبط الموضع داخل الدفق الحالي. |
...
| [to_bytes()](#to_bytes__12) | يحوّل بيانات الدفق إلى مصفوفة من نوع int. |
| [to_bytes(position, bytes_count)](#to_bytes_position_bytes_count_13) | يحوّل بيانات الدفق إلى مصفوفة من نوع int. |
| [write(buffer, offset, count)](#write_buffer_offset_count_14) | يكتب تسلسلًا من البايتات إلى الدفق الحالي ويقدّم الموضع الحالي داخل هذا الدفق بعدد البايتات المكتوبة. |
| [write(bytes)](#write_bytes_15) | يكتب جميع البايتات المحددة إلى الدفق. |
| [write_byte(value)](#write_byte_value_16) | يكتب بايتًا إلى الموضع الحالي في الدفق ويقدّم الموضع داخل الدفق بايتًا واحدًا. |
| [write_to(stream_container)](#write_to_stream_container_17) | ينسخ البيانات المحتواة إلى [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/) آخر. |
| [write_to(stream_container, length)](#write_to_stream_container_length_18) | ينسخ البيانات المحتواة إلى [StreamContainer](/psd/python-net/aspose.psd/streamcontainer/) آخر. |
...
يقوم بتهيئة نسخة جديدة من الفئة [SplitStreamContainer](/psd/python-net/aspose.psd/splitstreamcontainer/).
...
| معامل | النوع | الوصف |
| :- | :- | :- |
| stream | [StreamContainer](/psd/python-net/aspose.psd/streamcontainer) | دفق البيانات. |
| dispose_stream | bool | إذا تم تعيينه إلى <c>true</c> سيتم التخلص من الدفق عندما يتم التخلص من الحاوية. |
...
| stream | [StreamContainer](/psd/python-net/aspose.psd/streamcontainer) | حاوية الدفق للإدراج. |
| dispose_stream | bool | إذا تم تعيينه إلى <c>true</c> يتم التخلص من الدفق. |
...
| :- | :- | :- |
| destination_stream | _io.BufferedRandom | الدفق لحفظ البيانات إليه. |
| buffer_size | int | حجم المخزن المؤقت. يُستخدم قيمة ReadWriteBytesCount بشكل افتراضي. |
| length | long | The length of the stream data to copy. By default the length is set to [SplitStreamContainer.length](/psd/python-net/aspose.psd/splitstreamcontainer/) value. |
