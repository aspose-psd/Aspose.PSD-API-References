---
title: "XmpMeta"
second_title: "Aspose.PSD för Java API-referens"
description: "Representerar xmpmeta."
type: docs
weight: 17
url: /sv/java/com.aspose.psd.xmp/xmpmeta/
---

**Inheritance:**
java.lang.Object, [com.aspose.psd.xmp.XmpElementBase](../../com.aspose.psd.xmp/xmpelementbase)

**All Implemented Interfaces:**
[com.aspose.psd.xmp.IXmlValue](../../com.aspose.psd.xmp/ixmlvalue), com.aspose.ms.System.IEquatable
```
public final class XmpMeta extends XmpElementBase implements IXmlValue, System.IEquatable<XmpElementBase>
```

Representerar xmpmeta. Valfritt. Syftet med detta element är att identifiera XMP-metadata i generell XML-text som kan innehålla andra icke-XMP-användningar av RDF.
## Konstruktörer

| Konstruktör | Beskrivning |
| --- | --- |
| [XmpMeta(String toolkitVersion)](#XmpMeta-java.lang.String-) | Initierar en ny instans av  XmpMeta  klassen. |
| [XmpMeta()](#XmpMeta--) | Initierar en ny instans av  XmpMeta  klassen. |
## Metoder

| Metod | Beskrivning |
| --- | --- |
| [addAttribute(String attribute, String value)](#addAttribute-java.lang.String-java.lang.String-) | Lägger till attributet. |
| [assign_internalized(XmpElementBase xmpElement)](#assign-internalized-com.aspose.psd.xmp.XmpElementBase-) | Tilldelar det angivna XMP-elementet till det aktuella. |
| [clearAttributes()](#clearAttributes--) | Tar bort alla attribut. |
| [deepClone_internalized()](#deepClone-internalized--) | Klonar den här instansen. |
| [equals(Object other)](#equals-java.lang.Object-) | Bestämmer om det angivna  System.Object  är lika med den här instansen. |
| [getAdobeXmpToolkit()](#getAdobeXmpToolkit--) | Hämtar eller sätter Adobe Xmp-verktygssatsens version. |
| [getAttribute(String attribute)](#getAttribute-java.lang.String-) | Hämtar attributet. |
| [getClass()](#getClass--) |  |
| [getXmlValue()](#getXmlValue--) | Konverterar XMP‑värdet till XML‑representationen. |
| [hashCode()](#hashCode--) | Returnerar en hashkod för den här instansen. |
| [isEquals(XmpElementBase other)](#isEquals-com.aspose.psd.xmp.XmpElementBase-) | Anger om det aktuella objektet är lika med ett annat objekt av samma typ. |
| [isEquals(XmpMeta other)](#isEquals-com.aspose.psd.xmp.XmpMeta-) | Anger om det aktuella objektet är lika med ett annat objekt av samma typ. |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [setAdobeXmpToolkit(String value)](#setAdobeXmpToolkit-java.lang.String-) | Hämtar eller sätter Adobe Xmp-verktygssatsens version. |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### XmpMeta(String toolkitVersion) {#XmpMeta-java.lang.String-}
```
public XmpMeta(String toolkitVersion)
```


Initierar en ny instans av  XmpMeta  klassen.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| toolkitVersion | java.lang.String | Adobe XMP-verktygssats version. |

### XmpMeta() {#XmpMeta--}
```
public XmpMeta()
```


Initierar en ny instans av  XmpMeta  klassen.

### addAttribute(String attribute, String value) {#addAttribute-java.lang.String-java.lang.String-}
```
public void addAttribute(String attribute, String value)
```


Lägger till attributet.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| attribut | java.lang.String | Attributet. |
| värde | java.lang.String | Värdet. |

### assign_internalized(XmpElementBase xmpElement) {#assign-internalized-com.aspose.psd.xmp.XmpElementBase-}
```
public void assign_internalized(XmpElementBase xmpElement)
```


Tilldelar det angivna XMP-elementet till det aktuella.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| xmpElement | [XmpElementBase](../../com.aspose.psd.xmp/xmpelementbase) | XMP-elementet. |

### clearAttributes() {#clearAttributes--}
```
public void clearAttributes()
```


Tar bort alla attribut.

### deepClone_internalized() {#deepClone-internalized--}
```
public final XmpElementBase deepClone_internalized()
```


Klonar den här instansen.

**Returns:**
[XmpElementBase](../../com.aspose.psd.xmp/xmpelementbase) - The cloned object
### equals(Object other) {#equals-java.lang.Object-}
```
public boolean equals(Object other)
```


Bestämmer om det angivna  System.Object  är lika med den här instansen.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| annan | java.lang.Object | Det  System.Object  att jämföra med den här instansen. |

**Returns:**
boolean -  true  om det angivna  System.Object  är lika med den här instansen; annars  false .
### getAdobeXmpToolkit() {#getAdobeXmpToolkit--}
```
public String getAdobeXmpToolkit()
```


Hämtar eller sätter Adobe Xmp-verktygssatsens version.

**Returns:**
java.lang.String
### getAttribute(String attribute) {#getAttribute-java.lang.String-}
```
public String getAttribute(String attribute)
```


Hämtar attributet.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| attribut | java.lang.String | Attributet. |

**Returns:**
java.lang.String - Returnerar attributet för angivet attributnamn.
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getXmlValue() {#getXmlValue--}
```
public String getXmlValue()
```


Konverterar XMP‑värdet till XML‑representationen.

**Returns:**
java.lang.String - Returnerar XMP-värdet konverterat till XML-representationen.
### hashCode() {#hashCode--}
```
public int hashCode()
```


Returnerar en hashkod för den här instansen.

**Returns:**
int - En hashkod för den här instansen, lämplig för användning i hash‑algoritmer och datastrukturer som en hashtabell.
### isEquals(XmpElementBase other) {#isEquals-com.aspose.psd.xmp.XmpElementBase-}
```
public boolean isEquals(XmpElementBase other)
```


Anger om det aktuella objektet är lika med ett annat objekt av samma typ.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| other | [XmpElementBase](../../com.aspose.psd.xmp/xmpelementbase) | Ett objekt att jämföra med detta objekt. |

**Returns:**
boolean - true om det aktuella objektet är lika med  other  parametern; annars false.
### isEquals(XmpMeta other) {#isEquals-com.aspose.psd.xmp.XmpMeta-}
```
public boolean isEquals(XmpMeta other)
```


Anger om det aktuella objektet är lika med ett annat objekt av samma typ.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| other | [XmpMeta](../../com.aspose.psd.xmp/xmpmeta) | Ett objekt att jämföra med detta objekt. |

**Returns:**
boolean - true om det aktuella objektet är lika med  other  parametern; annars false.
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### setAdobeXmpToolkit(String value) {#setAdobeXmpToolkit-java.lang.String-}
```
public void setAdobeXmpToolkit(String value)
```


Hämtar eller sätter Adobe Xmp-verktygssatsens version.

**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| värde | java.lang.String |  |

### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final void wait(long arg0)
```




**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

