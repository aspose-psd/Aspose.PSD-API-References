---
title: XmpDynamicMediaPackage Class
type: docs
weight: 70
url: /python-net/aspose.psd.xmp.schemas.xmpdm/xmpdynamicmediapackage/
---

**Summary:** Represents XMP Dynamic Media namespace.

**Module:** [aspose.psd.xmp.schemas.xmpdm](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/)

**Full Name:** aspose.psd.xmp.schemas.xmpdm.XmpDynamicMediaPackage

**Inheritance:** IXmlValue, XmpPackage

**Aspose.PSD Version:** 23.12.0

## **Constructors**
| **Name** | **Description** |
| :- | :- |
| [XmpDynamicMediaPackage()](#XmpDynamicMediaPackage__1) | Initializes a new instance of the XmpDynamicMediaPackage class |
## **Properties**
| **Name** | **Type** | **Access** | **Description** |
| :- | :- | :- | :- |
| namespace_uri | string | r | Gets the namespace URI. |
| prefix | string | r | Gets the prefix. |
| xml_namespace | string | r | Gets the XML namespace. |
## **Methods**
| **Name** | **Description** |
| :- | :- |
| [add_value(key, value)](#add_value_key_value_1) | Adds string property. |
| clear() | Clears this instance. |
| [contains_key(key)](#contains_key_key_2) | Determines whether the specified key contains key. |
| [get_xml_value()](#get_xml_value__3) | Converts XMP value to the XML representation. |
| [remove(key)](#remove_key_4) | Remove the value with the specified key. |
| [set_abs_peak_audio_file_path(uri)](#set_abs_peak_audio_file_path_uri_5) | Sets the absolute peak audio file path. |
| [set_alblum(album)](#set_alblum_album_6) | Sets the alblum. |
| [set_alt_tape_name(alt_tape_name)](#set_alt_tape_name_alt_tape_name_7) | Sets the alternative tape name. |
| [set_alt_time_code(timecode)](#set_alt_time_code_timecode_8) | Sets the alternative time code. |
| [set_artist(artist)](#set_artist_artist_9) | Sets the artist. |
| [set_audio_channel_type(audio_channel_type)](#set_audio_channel_type_audio_channel_type_10) | Sets the audio channel type. |
| [set_audio_sample_rate(rate)](#set_audio_sample_rate_rate_11) | Sets the audio sample rate. |
| [set_audio_sample_type(audio_sample_type)](#set_audio_sample_type_audio_sample_type_12) | Sets the audio sample type. |
| [set_camera_angle(camera_angle)](#set_camera_angle_camera_angle_13) | Sets the camera angle. |
| [set_camera_label(camera_label)](#set_camera_label_camera_label_14) | Sets the camera label. |
| [set_camera_move(camera_move)](#set_camera_move_camera_move_15) | Sets the camera move. |
| [set_client(client)](#set_client_client_16) | Sets the client. |
| [set_comment(comment)](#set_comment_comment_17) | Sets the comment. |
| [set_composer(composer)](#set_composer_composer_18) | Sets the composer. |
| [set_director(director)](#set_director_director_19) | Sets the director. |
| [set_director_photography(director_photography)](#set_director_photography_director_photography_20) | Sets the director of photography. |
| [set_duration(duration)](#set_duration_duration_21) | Sets the duration. |
| [set_engineer(engineer)](#set_engineer_engineer_22) | Sets the engineer. |
| [set_file_data_rate(rate)](#set_file_data_rate_rate_23) | Sets the file data rate. |
| [set_genre(genre)](#set_genre_genre_24) | Sets the genre. |
| [set_good(good)](#set_good_good_25) | Sets the good. |
| [set_instrument(instrument)](#set_instrument_instrument_26) | Sets the instrument. |
| [set_intro_time(intro_time)](#set_intro_time_intro_time_27) | Sets the intro time. |
| [set_key(key)](#set_key_key_28) | Sets the audio’s musical key. |
| [set_log_comment(comment)](#set_log_comment_comment_29) | Sets the user's log comment. |
| [set_value(key, value)](#set_value_key_value_30) | Sets the value. |
| [set_xmp_type_value(key, value)](#set_xmp_type_value_key_value_31) | Sets the XMP type value. |


### Constructor: XmpDynamicMediaPackage() {#XmpDynamicMediaPackage__1}


```
 XmpDynamicMediaPackage() 
```

Initializes a new instance of the XmpDynamicMediaPackage class

### Method: add_value(key, value) {#add_value_key_value_1}


```
 add_value(key, value) 
```

Adds string property.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The string representation of key that is identified with added value. |
| value | string | The string value. |

### Method: contains_key(key) {#contains_key_key_2}


```
 contains_key(key) 
```

Determines whether the specified key contains key.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The key to be checked. |

**Returns**

| Type | Description |
| :- | :- |
| bool | Returns true if the specified key contains key. |


### Method: get_xml_value() {#get_xml_value__3}


```
 get_xml_value() 
```

Converts XMP value to the XML representation.

**Returns**

| Type | Description |
| :- | :- |
| string | Returns the XMP value converted to the XML representation. |


### Method: remove(key) {#remove_key_4}


```
 remove(key) 
```

Remove the value with the specified key.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The string representation of key that is identified with removed value. |

**Returns**

| Type | Description |
| :- | :- |
| bool | Returns true if the value with the specified key was removed. |


### Method: set_abs_peak_audio_file_path(uri) {#set_abs_peak_audio_file_path_uri_5}


```
 set_abs_peak_audio_file_path(uri) 
```

Sets the absolute peak audio file path.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| uri | string | The absolute path to the file’s peak audio file. |

### Method: set_alblum(album) {#set_alblum_album_6}


```
 set_alblum(album) 
```

Sets the alblum.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| album | string | The album. |

### Method: set_alt_tape_name(alt_tape_name) {#set_alt_tape_name_alt_tape_name_7}


```
 set_alt_tape_name(alt_tape_name) 
```

Sets the alternative tape name.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| alt_tape_name | string | Alternative tape name. |

### Method: set_alt_time_code(timecode) {#set_alt_time_code_timecode_8}


```
 set_alt_time_code(timecode) 
```

Sets the alternative time code.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| timecode | [Timecode](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/timecode) | Time code. |

### Method: set_artist(artist) {#set_artist_artist_9}


```
 set_artist(artist) 
```

Sets the artist.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| artist | string | The artist. |

### Method: set_audio_channel_type(audio_channel_type) {#set_audio_channel_type_audio_channel_type_10}


```
 set_audio_channel_type(audio_channel_type) 
```

Sets the audio channel type.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| audio_channel_type | [AudioChannelType](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/audiochanneltype) | Audio channel type. |

### Method: set_audio_sample_rate(rate) {#set_audio_sample_rate_rate_11}


```
 set_audio_sample_rate(rate) 
```

Sets the audio sample rate.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| rate | int | The audio sample rate. |

### Method: set_audio_sample_type(audio_sample_type) {#set_audio_sample_type_audio_sample_type_12}


```
 set_audio_sample_type(audio_sample_type) 
```

Sets the audio sample type.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| audio_sample_type | [AudioSampleType](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/audiosampletype) | The audio sample type. |

### Method: set_camera_angle(camera_angle) {#set_camera_angle_camera_angle_13}


```
 set_camera_angle(camera_angle) 
```

Sets the camera angle.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| camera_angle | string | The camera angle. |

### Method: set_camera_label(camera_label) {#set_camera_label_camera_label_14}


```
 set_camera_label(camera_label) 
```

Sets the camera label.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| camera_label | string | The camera label. |

### Method: set_camera_move(camera_move) {#set_camera_move_camera_move_15}


```
 set_camera_move(camera_move) 
```

Sets the camera move.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| camera_move | string | The camera move. |

### Method: set_client(client) {#set_client_client_16}


```
 set_client(client) 
```

Sets the client.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| client | string | The client. |

### Method: set_comment(comment) {#set_comment_comment_17}


```
 set_comment(comment) 
```

Sets the comment.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| comment | string | The comment. |

### Method: set_composer(composer) {#set_composer_composer_18}


```
 set_composer(composer) 
```

Sets the composer.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| composer | string | The composer. |

### Method: set_director(director) {#set_director_director_19}


```
 set_director(director) 
```

Sets the director.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| director | string | The director. |

### Method: set_director_photography(director_photography) {#set_director_photography_director_photography_20}


```
 set_director_photography(director_photography) 
```

Sets the director of photography.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| director_photography | string | The director of photography. |

### Method: set_duration(duration) {#set_duration_duration_21}


```
 set_duration(duration) 
```

Sets the duration.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| duration | [Time](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/time) | The duration. |

### Method: set_engineer(engineer) {#set_engineer_engineer_22}


```
 set_engineer(engineer) 
```

Sets the engineer.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| engineer | string | The engineer. |

### Method: set_file_data_rate(rate) {#set_file_data_rate_rate_23}


```
 set_file_data_rate(rate) 
```

Sets the file data rate.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| rate | [Rational](/psd/python-net/aspose.psd.xmp.types.derived/rational/) | The file data rate in megabytes per second. |

### Method: set_genre(genre) {#set_genre_genre_24}


```
 set_genre(genre) 
```

Sets the genre.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| genre | string | The genre. |

### Method: set_good(good) {#set_good_good_25}


```
 set_good(good) 
```

Sets the good.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| good | bool | if set to <c>true</c> a shot is a keeper. |

### Method: set_instrument(instrument) {#set_instrument_instrument_26}


```
 set_instrument(instrument) 
```

Sets the instrument.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| instrument | string | The instrument. |

### Method: set_intro_time(intro_time) {#set_intro_time_intro_time_27}


```
 set_intro_time(intro_time) 
```

Sets the intro time.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| intro_time | [Time](/psd/python-net/aspose.psd.xmp.schemas.xmpdm/time) | The intro time. |

### Method: set_key(key) {#set_key_key_28}


```
 set_key(key) 
```

Sets the audio’s musical key.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The audio’s musical key. One of: C, C#, D, D#, E, F, F#, G, G#, A, A#, and B. |

### Method: set_log_comment(comment) {#set_log_comment_comment_29}


```
 set_log_comment(comment) 
```

Sets the user's log comment.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| comment | string | The comment. |

### Method: set_value(key, value) {#set_value_key_value_30}


```
 set_value(key, value) 
```

Sets the value.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The string representation of key that is identified with added value. |
| value | [IXmlValue](/psd/python-net/aspose.psd.xmp/ixmlvalue) | The value to add to. |

### Method: set_xmp_type_value(key, value) {#set_xmp_type_value_key_value_31}


```
 set_xmp_type_value(key, value) 
```

Sets the XMP type value.

**Parameters:**

| Parameter | Type | Description |
| :- | :- | :- |
| key | string | The string representation of key that is identified with set value. |
| value | [XmpTypeBase](/psd/python-net/aspose.psd.xmp.types/xmptypebase/) | The value to set to. |

