---
title: Layer.Flags
second_title: Aspose.PSD for .NET API Reference
description: Layer property. Gets or sets the layer flags. bit 0  transparency protected bit 1  visible bit 2  obsolete bit 3  1 for Photoshop 5.0 and later tells if bit 4 has useful information bit 4  pixel data irrelevant to appearance of document
type: docs
weight: 140
url: /net/aspose.psd.fileformats.psd.layers/layer/flags/
---
{{< psd/tize >}}
## Layer.Flags property

Gets or sets the layer flags. bit 0 = transparency protected; bit 1 = visible; bit 2 = obsolete; bit 3 = 1 for Photoshop 5.0 and later, tells if bit 4 has useful information; bit 4 = pixel data irrelevant to appearance of document.

```csharp
public LayerFlags Flags { get; set; }
```

### Property Value

The layer flags.

### See Also

* enum [LayerFlags](../../layerflags/)
* class [Layer](../)
* namespace [Aspose.PSD.FileFormats.Psd.Layers](../../layer/)
* assembly [Aspose.PSD](../../../)


